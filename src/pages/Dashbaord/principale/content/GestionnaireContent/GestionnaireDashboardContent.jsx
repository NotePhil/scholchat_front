...
const GestionnaireDashboardContent = ({
  isDark,
  currentTheme,
  themes,
  colorSchemes,
    ...
    try {
      ...
      const [establishments, classes] = await Promise.allSettled([
        establishmentService
          .getEstablishmentsByGestionnaire(userId)
          .catch((err) => {
      ...
      const gestionnaireEstablishments =
        establishments.status === "fulfilled" ? establishments.value || [] : [];
      let allClasses =
  ...
  const establishmentDistributionData = dashboardData.establishments.map(
    (establishment, index) => ({
      name: establishment.nom || `Établissement ${index + 1}`,
      classes: dashboardData.classes.filter(
        (c) => c.etablissement?.id === establishment.id,
      ).length,
  ...
  const StatCard = ({
    ...
    >
      <div
        className={`absolute top-0 right-0 w-32 h-32 -mr-8 -mt-8 opacity-10 rounded-full ${color}`}
      ></div>
      ...
      >
        ...
        >
          <div
            className={`${isMobile ? "p-2 rounded-xl" : "p-4 rounded-2xl"} ${color} shadow-lg group-hover:scale-110 transition-transform ${isMobile ? "order-2" : ""}`}
          >
            <Icon
              className={`${isMobile ? "h-4 w-4" : "h-7 w-7"} text-white`}
            />
          </div>
          {isMobile && (
            <p
              className={`text-[10px] font-semibold uppercase tracking-wider ${isDark ? "text-gray-400" : "text-gray-500"} order-1`}
            >
              {title}
            </p>
          )}
        ...
        <div className="flex-1">
          {!isMobile && (
            <p
              className={`text-sm font-semibold uppercase tracking-wider ${isDark ? "text-gray-400" : "text-gray-500"}`}
            >
              {title}
            </p>
          )}
          <p
            className={`${isMobile ? "text-xl" : "text-4xl"} font-extrabold ${isDark ? "text-white" : "text-gray-900"} ${isMobile ? "" : "mt-2"}`}
          >
            {typeof value === "number" ? value.toLocaleString() : value}
          </p>
          {subtitle && (
            <p
              className={`${isMobile ? "text-[10px]" : "text-sm"} font-medium ${isDark ? "text-blue-400" : "text-blue-600"} ${isMobile ? "mt-1" : "mt-2"}`}
            >
              {subtitle}
            </p>
          )}
      ...
      {trend && !isMobile && (
        <div className="flex items-center mt-4 text-sm font-semibold text-green-500">
          <FontAwesomeIcon icon={faArrowTrendUp} className="h-4 w-4 mr-1" />
          <span>{trend}</span>
  ...
  const RecentActivity = () => {
    ...
    const recentActivities = [
      ...
      },
      {
        ...
        user:
          dashboardData.classes.find((c) => c.etat === "ACTIF")?.nom ||
    ...
    return (
      <div
        className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} rounded-2xl shadow-sm border p-6`}
      >
        ...
        <div className="space-y-6">
          {recentActivities.map((activity) => (
            <div
              key={activity.id}
              className="flex items-center group cursor-pointer hover:bg-gray-50 p-2 -m-2 rounded-xl transition-colors"
            >
              <div
                className={`flex-shrink-0 w-12 h-12 ${isDark ? "bg-gray-700" : "bg-gray-100"} rounded-2xl flex items-center justify-center group-hover:scale-110 transition-transform`}
              >
                <activity.icon className={`h-6 w-6 ${activity.color}`} />
              </div>
              <div className="flex-1 ml-4 min-w-0">
                <p
                  className={`text-sm font-bold ${isDark ? "text-white" : "text-gray-900"} truncate`}
                >
                  {activity.title}
                </p>
                <p
                  className={`text-xs ${isDark ? "text-gray-400" : "text-gray-500"} mt-1 truncate`}
                >
                  {activity.user}
                </p>
              </div>
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="text-center">
          ...
          <p
            className={`mt-6 font-medium ${isDark ? "text-gray-300" : "text-gray-600"}`}
          >
  ...
  if (dashboardData.error) {
    return (
      <div className="flex items-center justify-center py-20">
        <div
          className={`${isDark ? "bg-gray-800" : "bg-white"} p-8 rounded-2xl shadow-xl max-w-md w-full text-center border border-red-100`}
        >
  ...
  return (
    <div>
      {/* Header */}
      <div
        className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} border-b relative overflow-hidden`}
      >
        {!isMobile && (
          <div className="absolute top-0 right-0 w-1/3 h-full bg-gradient-to-l from-blue-50 to-transparent opacity-50"></div>
        )}
        <div
          className={`max-w-7xl mx-auto ${isMobile ? "px-3" : "px-4 sm:px-6 lg:px-8"} relative z-10`}
        >
          <div
            className={`flex ${isMobile ? "items-center justify-between py-3" : "flex-col md:flex-row md:justify-between md:items-center py-10"}`}
          >
            <div className={isMobile ? "flex-1 min-w-0" : ""}>
              {isMobile ? (
                <>
                  <div className="flex items-center gap-2">
                    <div className="px-2 py-0.5 rounded-full bg-blue-100 text-blue-700 text-[9px] font-bold uppercase">
                      Live
                    </div>
                    <h1
                ...
                <>
                  ...
                  <h1
                    className={`text-4xl font-black ${isDark ? "text-white" : "text-gray-900"} tracking-tight`}
                  >
                    Hello, {localStorage.getItem("username") || "Gestionnaire"}
                  </h1>
                  <p
                    className={`text-lg ${isDark ? "text-gray-300" : "text-gray-500"} mt-1 font-medium`}
                  >
                    Gérez vos {stats.totalEstablishments} établissements et
                    supervisez vos classes en temps réel.
                  </p>
                </>
              )}
            </div>
            <div className={isMobile ? "" : "mt-6 md:mt-0"}>
              ...
              >
                <FontAwesomeIcon
                  icon={faArrowsRotate}
                  className={`${isMobile ? "h-4 w-4" : "h-5 w-5"} ${loading ? "animate-spin text-blue-600" : "text-gray-400"}`}
                />
      ...
      <div
        className={`max-w-7xl mx-auto ${isMobile ? "px-3 py-3" : "px-4 sm:px-6 lg:px-8 py-10"}`}
      >
        {/* Main Stats Grid */}
        <div
          className={`${isMobile ? "hidden" : "grid"} grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-4 sm:mb-10`}
        >
          <StatCard
        ...
        <div
          className={`grid grid-cols-1 ${isMobile ? "gap-3" : "lg:grid-cols-2 gap-8"} mb-4 sm:mb-10`}
        >
          {/* Distribution Chart */}
          <div
            className={`${isDark ? "bg-gray-800 border-gray-700" : "bg-white border-gray-100"} ${isMobile ? "rounded-xl p-3" : "rounded-3xl shadow-sm border p-8"}`}
          >
            <div className="flex items-center justify-between mb-3 sm:mb-8">
              <div>
                <h3
                  className={`${isMobile ? "text-sm" : "text-xl"} font-bold ${isDark ? "text-white" : "text-gray-900"}`}
                >
                  Distribution des Classes
                </h3>
                {!isMobile && (
                  <p className="text-sm text-gray-500 mt-1">
                    Nombre de classes par établissement
                  </p>
                )}
              </div>
              <div
                className={`${isMobile ? "p-1.5" : "p-3"} bg-gray-50 text-gray-400 rounded-xl`}
              >
                <FontAwesomeIcon
                  icon={faChartBar}
          ...
          >
            <div className="flex items-center justify-between mb-3 sm:mb-8">
              <div>
                ...
                {!isMobile && (
                  <p className="text-sm text-gray-500 mt-1">
                    Proportion des classes actives vs en attente
                  </p>
                )}
              </div>
            </div>
            <div className={isMobile ? "h-40" : "h-80"}>
              ...
              <div
                className={`flex justify-center ${isMobile ? "gap-4 mt-1" : "gap-6 mt-4"}`}
              >
        ...
        <div
          className={`grid grid-cols-1 ${isMobile ? "gap-3" : "lg:grid-cols-2 gap-8"}`}
        >
          <RecentActivity />
          ...
          >
            <h3
              className={`${isMobile ? "text-sm" : "text-xl"} font-bold ${isDark ? "text-white" : "text-gray-900"} ${isMobile ? "mb-3" : "mb-8"}`}
            >
            ...
            >
              ...
              >
                <div
                  className={`${isMobile ? "w-8 h-8 rounded-lg mb-2" : "w-12 h-12 rounded-xl mb-4"} bg-white text-blue-600 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                >
                  <FontAwesomeIcon
                    icon={faSchool}
                    className={`${isMobile ? "h-4 w-4" : "h-6 w-6"}`}
              ...
              >
                <div
                  className={`${isMobile ? "w-8 h-8 rounded-lg mb-2" : "w-12 h-12 rounded-xl mb-4"} bg-white text-emerald-600 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                >
                  <FontAwesomeIcon
                    icon={faBuilding}
                    className={`${isMobile ? "h-4 w-4" : "h-6 w-6"}`}
                ...
                <span
                  className={`text-emerald-900 group-hover:text-white font-bold block ${isMobile ? "text-xs" : ""}`}
                >
                  Créer une Classe
                </span>
                {!isMobile && (
                  <span className="text-emerald-600 group-hover:text-emerald-100 text-xs mt-1 block">
                    Lancer un programme
                  </span>
                )}
              </button>
              ...
              >
                <div
                  className={`${isMobile ? "w-8 h-8 rounded-lg mb-2" : "w-12 h-12 rounded-xl mb-4"} bg-white text-indigo-600 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform`}
                >
                  <FontAwesomeIcon
                    icon={faUsers}
                    className={`${isMobile ? "h-4 w-4" : "h-6 w-6"}`}
                ...
                <span
                  className={`text-indigo-900 group-hover:text-white font-bold block ${isMobile ? "text-xs" : ""}`}
                >
                  Gérer les Élèves
                </span>
                {!isMobile && (
                  <span className="text-indigo-600 group-hover:text-indigo-100 text-xs mt-1 block">
                    Superviser les inscrits
                  </span>
                )}
              </button>
              ...
              >
                <div
                  className={`${isMobile ? "w-8 h-8 rounded-lg mb-2" : "w-12 h-12 rounded-xl mb-4"} bg-white text-amber-600 flex items-center justify-center shadow-sm group-hover:scale-110 transition-transform relative`}
                >
                  <FontAwesomeIcon
                ...
                <span
                  className={`text-amber-900 group-hover:text-white font-bold block ${isMobile ? "text-xs" : ""}`}
                >
                  Approbations
                </span>
                <span
                  className={`text-amber-600 group-hover:text-amber-100 ${isMobile ? "text-[10px]" : "text-xs"} mt-1 block`}
                >
                  {stats.pendingClasses} en attente
                </span>
              </button>
            </div>
          </div>
