...
const ManageClassContent = ({ onBack, tabData, setActiveTab }) => {
  ...
  const currentUserRole = (localStorage.getItem("userRole") || "")
    ...
    .replace("ROLE_", "");
  const isAdmin =
    currentUserRole === "ADMIN" || currentUserRole === "ADMINISTRATEUR";
  ...
  const getUserId = () => {
    ...
    if (!userId) {
      ...
      setError(
        t("classes.manage.errorAuth", "Erreur: Utilisateur non authentifié"),
      );
      return null;
    }
    ...
    try {
      ...
      if (selectedRole.includes("ADMIN")) {
        ...
        try {
          ...
          const etabResp = await fetch(
            `${process.env.REACT_APP_API_BASE_URL}/etablissements`,
            {
              headers: {
          ...
          if (etabResp.ok) {
            ...
            const myEtabIds = allEtabs
              .filter((e) => e.gestionnaireId === userId)
              .map((e) => e.id);
            setClasses(
              (allClasses || []).filter((c) =>
                myEtabIds.includes(c.etablissementId),
              ),
            );
          } else {
            setClasses([]);
        ...
        const [detailResult, accResult] = await Promise.allSettled([
          ...
          classService.axiosRequest(`/acceder/utilisateurs/${userId}/classes`, {
            method: "get",
          }),
        ]);
        ...
        const detailList =
          ...
          Array.isArray(detailResult.value)
            ? detailResult.value
            : [];

        // All accessible classes (members + rights)
        const accClasses =
          accResult.status === "fulfilled" && Array.isArray(accResult.value)
            ? accResult.value
            : [];
        ...
        const pubIds = new Set(
          detailList.map((d) => d.classe?.id).filter(Boolean),
        );
  ...
  const handleRefresh = async () => {
    ...
    setRefreshing(false);
    setSuccessMessage(
      t("classes.manage.successRefresh", "Données actualisées avec succès"),
    );
  };
  ...
  const handleJoinSearch = async () => {
    ...
    try {
      ...
      const found = (all || []).find((c) => c.codeActivation === code);
  ...
  const handleRequestAccess = async (cls, activationCode) => {
    try {
      ...
      handleClearJoinSearch();
      setError(
        err.message || "Une erreur est survenue lors de la demande d'accès",
      );
      throw err;
    }
  };
  ...
  const handleNavigateToExerciseManagement = (classId) => {
    ...
    if (setActiveTab) {
      setActiveTab("schedule-exercise", {
        classId,
      });
    } else {
      message.warning("Navigation non disponible");
        ...
        {!selectedClassId ? (
          <div className="w-full px-3 sm:px-6 py-3 sm:py-6">
              ...
              <div className="flex items-center gap-3 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-indigo-500 mt-0.5 hidden sm:block">
                    Recherchez une classe par nom, niveau ou code et demandez
                    l'accès.
                  </p>
                </div>
              </div>
            ...
            <ManageClassList
              ...
              onBack={onBack}
              onNavigateToCreate={
                setActiveTab ? () => setActiveTab("create-class") : undefined
              }
          ...
          <ManageClassDetailsView
            ...
            onNavigateToCourseCreation={
              isAdmin ? undefined : handleNavigateToCourseCreation
            }
            onNavigateToExerciseManagement={
              isAdmin ? undefined : handleNavigateToExerciseManagement
            }
            onNavigateToCoursManagement={
              isAdmin ? undefined : handleNavigateToCoursManagement
            }
        ...
        >
          <Input
            ...
            onChange={(e) => {
              ...
              if (!e.target.value) handleClearJoinSearch();
            }}
