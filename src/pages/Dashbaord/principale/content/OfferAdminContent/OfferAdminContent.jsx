...
const DUREE_PRESETS = [
  {
    label: "1 mois (30 jours)",
    minutes: 30 * 24 * 60,
  },
  {
    label: "1 an (365 jours)",
    minutes: 365 * 24 * 60,
  },
  {
    label: "2 ans",
...
const OfferAdminContent = ({ isDark }) => {
  ...
  const inputCls = `w-full px-3 py-2 rounded-lg border ${isDark ? "bg-gray-900 border-gray-600 text-gray-100" : "bg-white border-gray-300 text-gray-900"} focus:outline-none focus:ring-2 focus:ring-blue-500`;
      ...
      const payload = {
        ...
        nombreClassesInclues:
          formData.cible === TypeCibleOffre.ETABLISSEMENT
            ? toNumberOrNull(formData.nombreClassesInclues)
            : null,
        classesBonus:
          formData.cible === TypeCibleOffre.ETABLISSEMENT
            ? toNumberOrNull(formData.classesBonus)
            : null,
        estTest: formData.estTest,
        actif: formData.actif,
        delaiRappelSuppressionMinutes: toNumberOrNull(
          formData.delaiRappelSuppressionMinutes,
        ),
        delaiSuppressionMinutes: toNumberOrNull(
          formData.delaiSuppressionMinutes,
        ),
  ...
  const handleDesactiver = async (offre) => {
    if (
      !window.confirm(
        `Désactiver l'offre "${offre.nom}" ? Elle ne sera plus proposée aux nouveaux abonnements.`,
      )
    ) {
      return;
    }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className={textMuted}>
            Gérez les offres proposées pour les classes indépendantes et les
            établissements. Visible uniquement par les administrateurs.
          </p>
        </div>
        <button
          ...
          >
            ...
            <div>
              ...
              <select
                ...
                onChange={handleChange}
              >
                <option value={TypeCibleOffre.CLASSE}>
                  Classe indépendante
                </option>
                <option value={TypeCibleOffre.ETABLISSEMENT}>
                  Établissement
                </option>
              </select>
            </div>
            <div className="flex items-center gap-4 pt-6">
              <label className="flex items-center gap-2 text-sm">
                <input
                  type="checkbox"
                  name="estTest"
                  checked={formData.estTest}
                  onChange={handleChange}
                />
                <span className={textMuted}>
                  Offre de test (visible avec badge "TEST")
                </span>
              </label>
              <label className="flex items-center gap-2 text-sm">
                <input
            ...
            <div
              className={`md:col-span-2 border-t pt-4 ${isDark ? "border-gray-700" : "border-gray-200"}`}
            >
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {DUREE_PRESETS.map((p) => (
                      <button
                        ...
                        key={p.label}
                        onClick={() =>
                          appliquerPreset("dureeMensuelleMinutes", p.minutes)
                        }
            ...
            <div className="md:col-span-2">
              <h3 className={`font-medium mb-2 ${textMain}`}>
                Périodicité annuelle (optionnelle)
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={`block text-sm mb-1 ${textMuted}`}>
                  ...
                  <div className="flex flex-wrap gap-2 mt-1">
                    {DUREE_PRESETS.map((p) => (
                      <button
                        ...
                        key={p.label}
                        onClick={() =>
                          appliquerPreset("dureeAnnuelleMinutes", p.minutes)
                        }
                        className="text-xs px-2 py-1 rounded bg-blue-50 text-blue-700 hover:bg-blue-100"
                      >
                        {p.label}
              ...
              {formData.prixMensuel &&
                formData.prixAnnuel &&
                formData.dureeMensuelleMinutes &&
                formData.dureeAnnuelleMinutes && (
                  <p className="text-sm text-green-600 mt-2">
                    {(() => {
                      const reduction = offerService.calculerReduction({
                        ...
                        dureeMensuelleMinutes: Number(
                          formData.dureeMensuelleMinutes,
                        ),
                        dureeAnnuelleMinutes: Number(
                          formData.dureeAnnuelleMinutes,
                        ),
                      });
            ...
            >
              <h3
                className={`font-medium mb-2 flex items-center gap-2 ${textMain}`}
              >
                <FontAwesomeIcon
                  icon={faClock}
                  style={{
              ...
              <p className={`text-xs mb-3 ${textMuted}`}>
                Défini par vous, modifiable à tout moment. Laissez vide pour
                désactiver la purge automatique sur cette offre.
              </p>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className={`block text-sm mb-1 ${textMuted}`}>
                    Rappel de suppression (minutes après expiration)
                  </label>
                  <input
                    className={inputCls}
                    type="number"
                    name="delaiRappelSuppressionMinutes"
                    value={formData.delaiRappelSuppressionMinutes}
                    onChange={handleChange}
                  />
                  <div className="flex flex-wrap gap-2 mt-1">
                ...
                <div>
                  <label className={`block text-sm mb-1 ${textMuted}`}>
                    Suppression définitive (minutes après expiration)
                  </label>
                  <input
                    className={inputCls}
                    type="number"
                    name="delaiSuppressionMinutes"
                    value={formData.delaiSuppressionMinutes}
                    onChange={handleChange}
                  />
                  <div className="flex flex-wrap gap-2 mt-1">
                    {DELAI_SUPPRESSION_PRESETS.map((p) => (
                      <button
                        ...
                        key={p.label}
                        onClick={() =>
                          appliquerPreset("delaiSuppressionMinutes", p.minutes)
                        }
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div className="flex items-center pt-6">
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      name="messagerieIncluse"
                      checked={formData.messagerieIncluse}
                      onChange={handleChange}
                    />
            ...
            {formData.cible === TypeCibleOffre.ETABLISSEMENT && (
              ...
              >
                <h3 className={`font-medium mb-2 ${textMain}`}>
                  Quota de classes (établissement)
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className={`block text-sm mb-1 ${textMuted}`}>
                      Nombre de classes incluses
                    </label>
                    <input
                      className={inputCls}
                      type="number"
                      name="nombreClassesInclues"
                      value={formData.nombreClassesInclues}
                      onChange={handleChange}
                    />
                  </div>
                  <div>
                    <label className={`block text-sm mb-1 ${textMuted}`}>
                      Classes bonus offertes
                    </label>
                    <input
                      className={inputCls}
                      type="number"
                      name="classesBonus"
                      value={formData.classesBonus}
                      onChange={handleChange}
                    />
                  </div>
                </div>
            ...
            <div className="md:col-span-2 flex justify-end gap-3 pt-2">
              <button
                ...
                className={`px-4 py-2 rounded-lg border ${isDark ? "border-gray-600 text-gray-300" : "border-gray-300 text-gray-700"}`}
              >
                Annuler
              </button>
              <button
                  ...
                  >
                    ...
                    <td className={`p-3 ${textMuted}`}>
                      <span className="flex items-center gap-1">
                        {offre.cible === TypeCibleOffre.ETABLISSEMENT ? (
                          <FontAwesomeIcon
                            icon={faSchool}
                            style={{
                              fontSize: 14,
                    ...
                    <td className={`p-3 ${textMuted}`}>
                      {offre.prixMensuel != null
                        ? `${offre.prixMensuel} FCFA / ${offre.dureeMensuelleMinutes} min`
                        : "-"}
                    </td>
                    <td className={`p-3 ${textMuted}`}>
                      {offre.prixAnnuel != null
                        ? `${offre.prixAnnuel} FCFA / ${offre.dureeAnnuelleMinutes} min`
                        : "-"}
                    </td>
                    <td className={`p-3 ${textMuted}`}>
                    ...
                    <td className="p-3">
                      <span
                        className={`text-xs px-2 py-0.5 rounded-full ${offre.actif ? "bg-green-100 text-green-700" : "bg-gray-200 text-gray-600"}`}
                      >
                    ...
                    <td className="p-3">
                      <div className="flex justify-end gap-2">
                        ...
                        {offre.actif && (
                          <button
                            onClick={() => handleDesactiver(offre)}
