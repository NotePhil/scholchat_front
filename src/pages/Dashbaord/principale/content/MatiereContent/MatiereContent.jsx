...
const MatiereContent = ({ isDark, currentTheme, themes, colorSchemes }) => {
  ...
  const loadMatieres = async () => {
    try {
      ...
      setMessage({
        text: t("subjects.messages.loadError"),
        type: "error",
      });
      setMatieres([]);
    } finally {
  ...
  const handleCreate = async () => {
    if (!formData.nom.trim()) {
      setMessage({
        text: "Le nom de la matière est requis",
        type: "error",
      });
      return;
    }
  ...
  if (!canView) {
    return (
      <div className="flex items-center justify-center py-20 p-6">
        <div
          className={`${cardBgClass} rounded-2xl shadow-lg border ${borderClass} p-8 text-center max-w-md`}
        >
          <FontAwesomeIcon
  ...
  return (
    <div className="full-bleed-page">
      <div className="w-full px-3 sm:px-6 py-3 sm:py-6">
        {/* Header */}
        <div
          className={`${cardBgClass} rounded-2xl shadow-sm border ${borderClass} p-4 sm:p-6 mb-4 sm:mb-6`}
        >
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div
                className={`w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-gradient-to-br ${colorSchemes[currentTheme]?.gradient || "from-blue-500 to-blue-600"} flex items-center justify-center shadow-md flex-shrink-0`}
              >
                <FontAwesomeIcon
                  icon={faBookOpen}
              ...
              <div className="min-w-0">
                <h1
                  className={`text-lg sm:text-2xl font-bold ${textClass} leading-tight`}
                >
                  {t("subjects.title")}
                </h1>
                <p
        ...
        {message.text && (
          <div
            className={`mb-4 p-3 rounded-xl border flex items-center gap-2 text-sm ${message.type === "success" ? "bg-green-50 border-green-200 text-green-700" : "bg-red-50 border-red-200 text-red-700"}`}
          >
        ...
        {/* Search + Refresh */}
        <div
          className={`${cardBgClass} rounded-2xl shadow-sm border ${borderClass} p-3 sm:p-4 mb-4 sm:mb-6`}
        >
          <div className="flex gap-2">
            <div className="flex-1 relative">
              <FontAwesomeIcon
                icon={faMagnifyingGlass}
                className={`absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 ${textSecondaryClass}`}
              />
              <input
                type="text"
        ...
        {/* Content */}
        <div
          className={`${cardBgClass} rounded-2xl shadow-sm border ${borderClass} overflow-hidden`}
        >
          {loading ? (
            ...
            <div className="p-10 text-center">
              <FontAwesomeIcon
                icon={faBookOpen}
                className={`w-12 h-12 ${textSecondaryClass} mx-auto mb-3`}
              />
              <h3 className={`text-base font-semibold ${textClass} mb-1`}>
                {searchTerm
                  ? t("subjects.search.noResults")
                  : t("subjects.search.empty")}
              </h3>
              <p className={`text-sm ${textSecondaryClass}`}>
                {searchTerm
            ...
            <>
                  ...
                  <div key={index} className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      <div className="flex-1 min-w-0">
                        <p
                          className={`font-semibold text-sm ${textClass} truncate`}
                        >
                          {matiere.nom}
                        </p>
                        {matiere.description && (
                          <p
                            className={`text-xs ${textSecondaryClass} mt-0.5 line-clamp-2`}
                          >
                            {matiere.description}
                          </p>
                        )}
                      ...
                      <span
                        className={`flex-shrink-0 px-2 py-0.5 rounded-full text-xs font-medium ${matiere.etat === "ACTIF" ? "bg-green-100 text-green-700" : "bg-red-100 text-red-700"}`}
                      >
                        {matiere.etat || "ACTIF"}
                      </span>
                    </div>
                    <div className="flex items-center justify-between mt-2">
                      <div
                        className={`flex items-center gap-1 text-xs ${textSecondaryClass}`}
                      >
                        <FontAwesomeIcon
                          icon={faCalendarDays}
                          className="w-3 h-3"
              ...
              <div className="hidden sm:block overflow-x-auto">
                <table className="w-full">
                  <thead
                    className={`${isDark ? "bg-gray-700" : "bg-gray-50"} border-b ${borderClass}`}
                  >
                    <tr>
                      <th
                        className={`px-6 py-4 text-left text-xs font-semibold ${textClass} uppercase tracking-wide`}
                      >
                        {t("subjects.table.subject")}
                      </th>
                      <th
                        className={`px-6 py-4 text-left text-xs font-semibold ${textClass} uppercase tracking-wide`}
                      >
                      ...
                      {canManage && (
                        <th
                          className={`px-6 py-4 text-right text-xs font-semibold ${textClass} uppercase tracking-wide`}
                        >
                          Actions
                        </th>
                      )}
                  ...
                  <tbody className={`divide-y ${borderClass}`}>
                    {currentMatieres.map((matiere, index) => (
                      <tr
                        key={index}
                        className={`hover:${isDark ? "bg-gray-700/50" : "bg-gray-50/60"} transition-colors`}
                      >
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-1.5">
                            <FontAwesomeIcon
                              icon={faCalendarDays}
                              className={`w-3.5 h-3.5 ${textSecondaryClass}`}
                            />
                        ...
                        <td className="px-6 py-4">
                          <span
                            className={`px-2.5 py-1 rounded-full text-xs font-medium ${matiere.etat === "ACTIF" ? `${isDark ? "bg-green-900/50 text-green-300" : "bg-green-100 text-green-700"}` : `${isDark ? "bg-red-900/50 text-red-300" : "bg-red-100 text-red-700"}`}`}
                          >
                            {matiere.etat || "ACTIF"}
                          </span>
                        </td>
          ...
          {showPagination && !loading && currentMatieres.length > 0 && (
            <div
              className={`px-4 py-3 border-t ${borderClass} flex items-center justify-between gap-2`}
            >
              <span className={`text-xs ${textSecondaryClass}`}>
                {startIndex + 1}–{Math.min(endIndex, filteredMatieres.length)} /{" "}
                ...
                {Array.from(
                  ...
                  .map((p, idx) =>
                    p === "…" ? (
                      <span
                        key={`e-${idx}`}
                        className={`px-1 text-xs ${textSecondaryClass}`}
                      >
                        …
                      </span>
                    ) : (
                      <button
                        ...
                        className={`w-7 h-7 rounded-lg text-xs font-medium transition-colors ${currentPage === p ? `bg-gradient-to-r ${colorSchemes[currentTheme]?.gradient || "from-blue-500 to-blue-600"} text-white` : `border ${borderClass} ${textClass} hover:bg-gray-50`}`}
                      >
                        {p}
                      </button>
                    ),
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[9999] p-4">
          <div
            className={`${cardBgClass} rounded-2xl shadow-xl w-full max-w-md max-h-[90vh] overflow-y-auto`}
          >
            <div className={`p-6 border-b ${borderClass}`}>
              <div className="flex items-center justify-between">
                <h2 className={`text-xl font-bold ${textClass}`}>
                  {t("subjects.modals.create")}
                </h2>
                <button
                  onClick={closeModals}
            ...
            <div className="p-6 space-y-4">
              <div>
                <label
                  className={`block text-sm font-semibold ${textClass} mb-2`}
                >
            ...
            <div
              className={`p-6 border-t ${borderClass} flex justify-end gap-3`}
            >
              <button
                onClick={closeModals}
          ...
          >
            ...
            <div className="p-6">
              <p className={textClass}>
                ...
                <strong>{selectedMatiere.nom}</strong> ?
              </p>
