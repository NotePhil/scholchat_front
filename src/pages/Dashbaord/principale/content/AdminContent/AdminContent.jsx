...
const Shield = asIconComponent(faShield);
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <div className="flex items-center space-x-3 mt-2">
                  <span
                    className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium border ${getStatusBadge(admin.etat)}`}
                  >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center space-x-4">
          ...
          <div className="flex-1 min-w-0">
            ...
            <span
              className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium border ${getStatusBadge(admin.etat)} mt-1`}
            >
              {getStatusText(admin.etat)}
            </span>
          </div>
...
const AdminContent = ({ isDark, currentTheme, themes, colorSchemes }) => {
  ...
  const language = useReduxSelector(
    (state) => state.ui?.currentLanguage || "fr",
  );
    ...
    return (
      <div className="flex items-center justify-center py-20">
        <div className="flex flex-col items-center space-y-4">
          ...
          />
          <p className="text-slate-600 font-medium">{t("admin.loading")}</p>
        </div>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="mb-8">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <div>
              <h1
                className={`text-3xl font-bold ${isDark ? "text-white" : "bg-gradient-to-r from-slate-900 to-slate-700 bg-clip-text text-transparent"}`}
              >
                {t("admin.title")}
              </h1>
        ...
        <div
          className={`${isMobile ? "hidden" : "grid"} grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8`}
        >
          <StatsCard
        ...
        <div
          className={`bg-white/70 backdrop-blur-sm border border-white/50 rounded-2xl ${isMobile ? "p-3" : "p-6"} shadow-lg mb-8`}
        >
          <div
            className={`flex ${isMobile ? "flex-col gap-3" : "flex-col lg:flex-row lg:items-center lg:justify-between gap-6"}`}
          >
            <div
            ...
            >
              ...
              <div
                className={`flex ${isMobile ? "justify-between" : ""} items-center gap-2`}
              >
                <div className="flex bg-slate-100 rounded-xl p-1">
                  <button
                    onClick={() => handleViewModeChange("grid")}
                    className={`${isMobile ? "px-3 py-1 text-xs" : "px-4 py-2 text-sm"} rounded-lg font-medium transition-all duration-200 ${viewMode === "grid" ? "bg-white text-purple-600 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}
                  >
                    {t("admin.search.grid")}
                  </button>
                  <button
                    onClick={() => handleViewModeChange("table")}
                    className={`${isMobile ? "px-3 py-1 text-xs" : "px-4 py-2 text-sm"} rounded-lg font-medium transition-all duration-200 ${viewMode === "table" ? "bg-white text-purple-600 shadow-sm" : "text-slate-600 hover:text-slate-900"}`}
                  >
        ...
        {viewMode === "grid" ? (
          <div
            className={`grid ${isMobile ? "grid-cols-1 gap-3" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6"}`}
          >
            ...
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-slate-200">
                <thead className="bg-slate-50/50">
                  <tr>
                    <th
                      className={`${isMobile ? "px-3 py-2" : "px-6 py-4"} text-left text-xs font-semibold text-slate-600 uppercase tracking-wider`}
                    >
                    ...
                    <th
                      className={`${isMobile ? "px-3 py-2" : "px-6 py-4"} text-right text-xs font-semibold text-slate-600 uppercase tracking-wider`}
                    >
                ...
                <tbody className="bg-white/30 divide-y divide-slate-100">
                  {safeFilteredAdmins.map((admin) => (
                    ...
                    >
                      <td
                        className={`${isMobile ? "px-3 py-2" : "px-6 py-4"} whitespace-nowrap`}
                      >
                        <div className="flex items-center">
                          <div
                            className={`${isMobile ? "h-8 w-8" : "h-10 w-10"} bg-gradient-to-r from-purple-500 to-indigo-500 rounded-xl flex items-center justify-center shadow-md`}
                          >
                            <span
                              className={`text-white font-medium ${isMobile ? "text-xs" : "text-sm"}`}
                            >
                              {`${admin.prenom?.charAt(0) || ""}${admin.nom?.charAt(0) || ""}`.toUpperCase()}
                            </span>
                          </div>
                          <div className={`${isMobile ? "ml-2" : "ml-4"}`}>
                            <div
                              className={`${isMobile ? "text-xs" : "text-sm"} font-semibold text-slate-900`}
                            >
                              {admin.prenom} {admin.nom}
                            </div>
                          </div>
                      ...
                      >
                        <div className="space-y-1">
                          <div
                            className={`${isMobile ? "text-xs" : "text-sm"} text-slate-900`}
                          >
                            {admin.email}
                          </div>
                          {admin.telephone && (
                            <div
                              className={`${isMobile ? "text-xs" : "text-sm"} text-slate-500`}
                            >
                              {admin.telephone}
                            </div>
                          )}
                      ...
                      >
                        <span
                          className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium border ${admin.etat === "ACTIVE" ? "bg-emerald-50 text-emerald-700 border-emerald-200" : admin.etat === "INACTIVE" ? "bg-red-50 text-red-700 border-red-200" : "bg-amber-50 text-amber-700 border-amber-200"}`}
                        >
                          {t(`admin.status.${admin.etat?.toLowerCase()}`) ||
                            admin.etat}
                        </span>
                      </td>
                      <td
                        className={`${isMobile ? "px-3 py-2" : "px-6 py-4"} whitespace-nowrap text-right text-sm`}
                      >
