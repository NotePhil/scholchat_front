...
const CourseContentView = ({ course, onBack }) => {
  ...
  const extractDocumentsFromChapters = (chapitres) => {
    ...
    const isProxied = (url) =>
      url && apiBase && url.startsWith(apiBase) && url.includes("/media/");
  ...
  const processChapterContent = async () => {
    ...
    try {
      ...
      setProcessedChapters(
        course.chapitres.map((ch) => ({
          ...ch,
          processedContent: ch.contenu || "",
        })),
      );
    }
  };
          ...
          <div className="relative px-3 sm:px-6 py-3 sm:py-4">
            ...
            <div className="flex items-center gap-2 mb-3">
              ...
              <button
                onClick={() =>
                  navigate(
                    "/schoolchat/Principal/ProfessorDashboard/schedule-course",
                    {
                      state: {
                        course,
                      },
            ...
            {[
              ...
              ...(processedChapters.length > 0 || course?.chapitres?.length > 0
                ? [
                    {
                      key: "chapters",
                      label: `Chapitres (${processedChapters.length || course?.chapitres?.length || 0})`,
                    },
                  ]
                : []),
              ...
              {
                key: "documents",
                label: `Documents (${courseDocuments.length})`,
              },
              {
                key: "history",
                label: "Historique",
              ...
              <button
                ...
                className={`flex-shrink-0 py-3 px-4 text-sm font-medium border-b-2 transition-all whitespace-nowrap ${activeTab === tab.key ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"}`}
              >
                {tab.label}
              </button>
            ))}
              ...
              {(course?.matiere || course?.matieres?.length > 0) && (
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  >
                    ...
                    />
                    <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">
                      {course?.matieres?.length > 1
                        ? "Matières Associées"
                        : "Matière Associée"}
                    </span>
                  </div>
                  <div className="p-4 sm:p-6 space-y-3">
                    {(course?.matiere ? [course.matiere] : course.matieres).map(
                      (m, i) => (
                        <div
                          key={m.id || i}
                        ...
                        >
                          ...
                          <span
                            className={`flex-shrink-0 text-xs px-2 py-0.5 rounded-full font-medium ${m.etat === "ACTIF" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"}`}
                          >
                            {m.etat || "ACTIF"}
                          </span>
                        </div>
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-3">
                          <div
                            className={`flex-shrink-0 w-10 h-10 rounded-lg flex items-center justify-center ${isImage ? "bg-purple-50" : "bg-blue-50"}`}
                          >
                            {isImage ? (
                              <img
                                src={doc.url}
                              ...
                              <FontAwesomeIcon
                                icon={faFileLines}
                                className={`w-5 h-5 ${isImage ? "text-purple-500" : "text-blue-500"}`}
                              />
                            )}
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <span
                              className={`inline-block mt-1 text-xs px-2 py-0.5 rounded-full ${isImage ? "bg-purple-100 text-purple-700" : "bg-blue-100 text-blue-700"}`}
                            >
                ...
                <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                  <div className="flex items-start space-x-4">
                    ...
                    <div className="flex-1">
                      <h4 className="font-medium text-slate-900">Cours créé</h4>
                      <p className="text-sm text-slate-600 mt-1">
