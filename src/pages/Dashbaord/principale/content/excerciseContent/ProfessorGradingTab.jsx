...
const ProfessorGradingTab = ({ exerciseId, exerciseProgrammerId }) => {
  ...
  const studentsWithAnswers = students.filter(
  ...
  const getStudentScore = (student) => {
    ...
    return {
      score,
      graded,
      total: student.answers.length,
    };
  };
  const averageScore =
    students.length > 0
      ? Math.round(
          (students.reduce((sum, s) => sum + getStudentScore(s).score, 0) /
            students.length) *
            10,
        ) / 10
      : 0;
    ...
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-400 text-sm">
          Les resultats apparaitront ici apres que les eleves aient soumis leurs
          reponses.
        </p>
      </div>
    );
    ...
    <div>
      ...
      <div className="hidden md:grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-3 text-center">
          <FontAwesomeIcon
            icon={faChartBar}
            className="w-5 h-5 text-purple-600 mx-auto mb-1"
          />
            ...
            >
              ...
              >
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <div
                    className={`px-3 py-1 rounded-full text-sm font-bold ${percentage >= 70 ? "bg-green-100 text-green-700" : percentage >= 40 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}
                  >
                    {score}/{maxPoints}
                  </div>
                  <span className="text-xs text-gray-400">
                    {student.answers.length}/{totalQuestions}
                  </span>
                  {isExpanded ? (
                    <FontAwesomeIcon
                      icon={faChevronUp}
                ...
                <div className="border-t bg-gray-50 p-4">
                  ...
                  {exerciseProgrammerId && (
                    <div className="mb-4 p-3 bg-white border rounded-lg">
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        >
                          {savingGrade === `overall-${student.id}` ? (
                            <FontAwesomeIcon
                              icon={faSpinner}
                  ...
                  <div className="space-y-3">
                    {questions.map((question, qIdx) => {
                      ...
                      const needsManualGrade =
                        answer && answer.estCorrecte === null;
                      const correctChoice = (question.choixReponses || []).find(
                        (c) => c.estCorrect,
                      );
                      return (
                        <div
                        ...
                        >
                          <div className="flex items-start gap-2 mb-2">
                            <span className="flex-shrink-0 w-6 h-6 rounded-full bg-gray-200 flex items-center justify-center text-xs font-bold text-gray-600">
                              {qIdx + 1}
                            </span>
                            <div className="flex-1 min-w-0">
                              ...
                              <span className="text-xs text-gray-500">
                                {question.typeQuestion} - {question.points || 1}{" "}
                                pt(s)
                              </span>
                            </div>
                              ...
                              {needsManualGrade && (
                                <div className="mt-2 p-2 bg-amber-50 border border-amber-200 rounded-lg">
                                  ...
                                  <div className="flex items-center gap-2 flex-wrap">
                                    ...
                                    <button
                                      onClick={() =>
                                        handleSaveGrade(student.id, question.id)
                                      }
