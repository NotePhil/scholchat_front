...
const ProgrammationCorrections = ({ prog }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const [freshProg, freshParticipations] = await Promise.all([
        exerciseProgrammerService
          .getExerciseProgrammeById(prog.id)
          .catch(() => prog),
        participationExerciseService
          .getParticipationsByExercise(prog.id)
          .catch(() => prog.participations || []),
      ]);
      ...
      const participantIds = [
        ...new Set(freshParticipations.map((p) => p.utilisateurId)),
      ];
    ...
    return (
      <div className="text-center py-10">
        ...
        <p className="text-gray-500 text-sm">
          Aucune soumission pour cette programmation
        </p>
      </div>
    );
  }
  ...
  const totalQuestionPages = (studentId) =>
    Math.ceil(questions.length / QUESTION_PAGE_SIZE);
    ...
    <div>
        ...
        {[
          ...
          },
          {
            ...
            value: students.filter(
              (s) => s.participation?.etatSoumission === "CORRIGE",
            ).length,
            color: "#7c3aed",
            bg: "#f5f3ff",
          },
              ...
              {isExpanded && (
                <div className="border-t bg-gray-50 p-4 space-y-3">
                  ...
                  >
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <Input
                        ...
                        value={
                          gradeInputs[`overall-${student.id}`]?.appreciation ||
                          ""
                        }
                        onChange={(e) =>
                          setInput(
                  ...
                  {paginatedQuestions(student.id).map((q, idx) => {
                    const globalIdx =
                      (getQuestionPage(student.id) - 1) * QUESTION_PAGE_SIZE +
                      idx;
                    const answer = student.answers.find(
                      (a) => a.questionId === q.id,
                    );
                    const gradeKey = `${student.id}-${q.id}`;
                    const isAutoType = ["QCM", "VRAI_FAUX"].includes(
                      q.typeQuestion,
                    );
                    const correctChoices = (q.choixReponses || []).filter(
                      (c) => c.estCorrect,
                    );
                    ...
                    return (
                      ...
                      >
                        ...
                        <div
                          ...
                          style={{
                            ...
                            background: needsGrading
                              ? "#fffbeb"
                              : alreadyGraded && answer.estCorrecte
                                ? "#f0fdf4"
                                : alreadyGraded
                                  ? "#fef2f2"
                                  : "#f9fafb",
                        ...
                        >
                          <span className="w-5 h-5 rounded-full bg-gray-300 flex items-center justify-center text-xs font-bold text-gray-700 flex-shrink-0">
                            {globalIdx + 1}
                          </span>
                        ...
                        <div className="px-3 py-2 space-y-2">
                          ...
                          {expectedAnswer ? (
                            <div className="flex items-start gap-2">
                              <span className="text-xs font-semibold text-green-700 w-28 flex-shrink-0 pt-0.5">
                                Réponse attendue
                              </span>
                              <span className="text-sm text-green-800 bg-green-50 border border-green-200 px-2 py-1 rounded flex-1">
                                {expectedAnswer}
                              </span>
                            </div>
                          ) : (
                          ...
                          {answer ? (
                            <div className="flex items-start gap-2">
                              ...
                              <span
                                className={`text-sm px-2 py-0.5 rounded flex-1 ${answer.estCorrecte === true ? "bg-green-50 text-green-800" : answer.estCorrecte === false ? "bg-red-50 text-red-800" : "bg-gray-50 text-gray-800"}`}
                              >
                          ...
                          {!isAutoType && answer && (
                            <div className="pt-1 border-t border-dashed border-gray-200">
                              ...
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                <button
                                  onClick={() =>
                                    setInput(gradeKey, "estCorrecte", true)
                                  }
                                  className={`px-3 py-1 rounded-lg text-xs font-semibold border transition-colors ${gradeInputs[gradeKey]?.estCorrecte === true ? "bg-green-500 text-white border-green-500" : "bg-white text-green-700 border-green-300 hover:bg-green-50"}`}
                                >
                                  ✓ Correct
                                </button>
                                <button
                                  onClick={() =>
                                    setInput(gradeKey, "estCorrecte", false)
                                  }
                                  className={`px-3 py-1 rounded-lg text-xs font-semibold border transition-colors ${gradeInputs[gradeKey]?.estCorrecte === false ? "bg-red-500 text-white border-red-500" : "bg-white text-red-700 border-red-300 hover:bg-red-50"}`}
                                >
                                  ✕ Incorrect
                                </button>
                                <Input
                                  size="small"
                                  placeholder={`Note (/${q.points || 1})`}
                                  value={gradeInputs[gradeKey]?.note || ""}
                                  onChange={(e) =>
                                    setInput(gradeKey, "note", e.target.value)
                                  }
                                  style={{
                                    width: 90,
                                  }}
                                ...
                                <Button
                                  ...
                                  disabled={
                                    gradeInputs[gradeKey]?.estCorrecte ===
                                      undefined && !gradeInputs[gradeKey]?.note
                                  }
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-gray-100">
                      <button
                        onClick={() =>
                          setQuestionPage(
                            student.id,
                            Math.max(1, getQuestionPage(student.id) - 1),
                          )
                        }
                      ...
                      <span className="text-xs text-gray-500">
                        Q{" "}
                        {(getQuestionPage(student.id) - 1) *
                          QUESTION_PAGE_SIZE +
                          1}
                        –
                        {Math.min(
                          getQuestionPage(student.id) * QUESTION_PAGE_SIZE,
                          questions.length,
                        )}{" "}
                        / {questions.length}
                      </span>
                      <button
                        onClick={() =>
                          setQuestionPage(
                            student.id,
                            Math.min(
                              totalQuestionPages(student.id),
                              getQuestionPage(student.id) + 1,
                            ),
                          )
                        }
                        disabled={
                          getQuestionPage(student.id) ===
                          totalQuestionPages(student.id)
                        }
                        className="px-2 py-1 text-xs rounded border border-gray-200 disabled:opacity-40 hover:bg-gray-50"
                      >
      ...
      {totalStudentPages > 1 && (
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-gray-100">
          ...
          <span className="text-xs text-gray-500">
            Page {studentPage} / {totalStudentPages} · {students.length} élève
            {students.length > 1 ? "s" : ""}
          </span>
          <button
            onClick={() => {
      ...
      <div className="divide-y divide-gray-50">
        {paginated.map((prog) => (
          <button
            ...
            className={`w-full text-left px-3 py-2.5 transition-colors ${selectedProgId === prog.id ? "bg-blue-50 border-l-2 border-blue-600" : "hover:bg-gray-50 border-l-2 border-transparent"}`}
          >
...
const ExerciseCorrectionsContent = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const sorted = Array.from(merged.values()).sort(
        (a, b) => new Date(b.dateExoPrevue) - new Date(a.dateExoPrevue),
      );
        ...
        sorted.map(async (prog) => {
          try {
            const participations =
              await participationExerciseService.getParticipationsByExercise(
                prog.id,
              );
  ...
  useEffect(() => {
    ...
    userService
      .getUserById(selectedProg.programmeParId)
      .then((user) => {
        if (user)
          setProfessorName(
        ...
        <div className="relative bg-gradient-to-r from-purple-600 to-indigo-700 px-4 py-3 flex flex-col sm:flex-row sm:items-center gap-3">
          ...
          <div className="relative flex items-center gap-4 sm:gap-5 flex-shrink-0">
            {[
              {
                label: "Total",
                value: programmations.length,
                color: "#c4b5fd",
              },
              {
                label: "Devoirs",
                value: programmations.filter(
                  (p) => p.typeAssignation === "DEVOIR",
                ).length,
                color: "#fca5a5",
              },
              {
      ...
      {programmations.length === 0 ? (
        <div className="bg-white rounded-xl shadow p-10 text-center">
          ...
          <p className="text-gray-400 text-sm">
            Programmez d'abord des exercices pour voir les corrections ici.
          </p>
        </div>
      ) : (
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {selectedProg && (
                    <p className="text-xs text-gray-400">
                      {selectedProg.typeAssignation === "DEVOIR"
                        ? "Devoir"
                        : "Exercice libre"}
                      {selectedProg.classesDiffusees?.length > 0 &&
                        ` · ${selectedProg.classesDiffusees.map((c) => c.nom).join(", ")}`}
                    </p>
                  )}
                </div>
              </div>
                ...
                <div className="px-4 py-2.5 bg-amber-50 border-b border-amber-100 flex items-center gap-2">
                  ...
                  <span className="text-xs text-amber-700 font-medium">
                    ...
                    <span className="font-bold">
                      {professorName || "un autre professeur"}
                    </span>{" "}
              ...
              <div className="p-4">
                {selectedProg ? (
                  ...
                  <div className="text-center py-10">
                    ...
                    />
                    <p className="text-gray-500 text-sm">
                      Sélectionnez une programmation à gauche
                    </p>
                  </div>
                )}
              </div>
