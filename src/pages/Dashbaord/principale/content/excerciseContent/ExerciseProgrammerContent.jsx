...
const getEffectiveEtat = (prog) => {
  if (
    (prog.etat === "ACTIF" || prog.etat === "PUBLIE") &&
...
const ExerciseProgrammerContent = () => {
    ...
    try {
      ...
      merged.forEach((p, key) => {
        if (!p.isOwn && p.programmeParId) {
          merged.set(key, {
            ...p,
            programmeParNom: professorNames[String(p.programmeParId)] || null,
          });
        }
      });
      const EXO_STATUS_ORDER = {
        EN_COURS: 0,
        PLANIFIE: 1,
        ANNULE: 2,
      };
  ...
  const filtered = programmations.filter((prog) => {
    const matchClass =
      ...
      (Array.isArray(prog.classesDiffusees) &&
        prog.classesDiffusees.some(
          (c) => String(c.id) === String(filterClassId),
        ));
    const matchSearch =
      !searchTerm ||
  ...
  const stats = {
    ...
    actif: programmations.filter(
      (p) =>
        getEffectiveEtat(p) === "ACTIF" || getEffectiveEtat(p) === "PUBLIE",
    ).length,
    devoirs: programmations.filter((p) => p.typeAssignation === "DEVOIR")
      .length,
    exercices: programmations.filter((p) => p.typeAssignation === "EXERCICE")
      .length,
  };
                    ...
                    <Option value="DEVOIR">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          ...
                          <div className="text-xs text-gray-400">
                            Correction manuelle du professeur
                          </div>
                        </div>
                      </div>
                    </Option>
                      ...
                      ({ getFieldValue }) => ({
                        validator(_, value) {
                          ...
                          return Promise.reject(
                            new Error("Doit être après le début"),
                          );
                        },
                      }),
                    ]}
      ...
      <div className="bg-white rounded-xl shadow-sm border border-gray-100 px-4 py-3 mb-3 flex flex-col gap-2">
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
          ...
          >
            <FontAwesomeIcon
              icon={faArrowsRotate}
              className={progsLoading ? "animate-spin" : ""}
              style={{
                fontSize: 14,
              }}
            />
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <div className="relative min-w-[160px]">
            ...
            <select
              value={filterClassId}
              onChange={(e) =>
                handleFilterChange(setFilterClassId)(e.target.value)
              }
                ...
                .map((p, i) =>
                  p === "…" ? (
                    ...
                    <button
                      ...
                      className={`px-2.5 py-1.5 text-xs rounded-lg border transition-colors ${p === safePage ? "bg-indigo-600 text-white border-indigo-600" : "border-gray-200 hover:bg-white hover:border-indigo-300 hover:text-indigo-600"}`}
                    >
                      {p}
                    </button>
                  ),
        ...
        {detailProg && (
          <div className="space-y-3 pt-2">
            {[
              {
                label: "Exercice",
                value: detailProg.nom || "—",
              },
              {
                label: "Type",
                value: <TypeBadge type={detailProg.typeAssignation} />,
              },
              {
                label: "Statut",
                value: <EtatBadge etat={getEffectiveEtat(detailProg)} />,
              },
              {
                label: "Date prévue",
                value: fmtDateTime(detailProg.dateExoPrevue),
              },
              {
                label: "Début effectif",
                value: fmtDateTime(detailProg.dateDebutExoEffectif),
              },
              {
                label: "Fin effective",
              ...
              },
              {
                label: "Classes",
                value:
                  detailProg.classesDiffusees?.length > 0
