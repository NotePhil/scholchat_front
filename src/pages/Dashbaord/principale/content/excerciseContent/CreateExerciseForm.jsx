...
const CreateExerciseForm = ({ onSubmit, onCancel, onError, onSuccess }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        setMatieres((await matiereService.getAllMatieres()) || []);
      } catch {
  ...
  const handleAddQuestion = () => {
    ...
    message.success(
      editingIndex !== null ? "Question mise à jour" : "Question ajoutée",
    );
  };
    ...
    try {
      ...
      if (created?.id) {
        for (const mId of values.matiereIds || [])
          try {
            await exerciseService.lierExerciseAMatiere(created.id, mId);
          } catch {}
        for (const q of questions) {
          try {
            await questionReponseService.createQuestion(
              created.id,
            ...
            failedQuestions += 1;
            console.error(
              "[CreateExerciseForm] failed to save question:",
              q,
              qErr,
            );
          }
        ...
        >
          ...
          <div>
            ...
            <div className="text-blue-100 text-sm opacity-90">
              Renseignez les informations et ajoutez vos questions
            </div>
          </div>
        </div>
        ...
        >
            ...
            >
              ...
              <Form.Item name="matiereIds" label="Matières associées">
                <Select
                  ...
                  notFoundContent={
                    loadingMatieres ? <Spin size="small" /> : "Aucune matière"
                  }
                >
                  {matieres.map((m) => (
            ...
            >
              ...
              >
                ...
                <div>
                  ...
                  <div className="text-xs text-gray-400">
                    Construisez les questions de votre exercice
                  </div>
                </div>
              </div>
          ...
          >
            ...
            <ul className="text-xs text-blue-800 space-y-0.5 list-none pl-0 m-0">
              <li>
                ① Choisissez le <strong>niveau</strong> correspondant à l'enum
                backend (Collège, Lycée…).
              </li>
              <li>
                ② Dans le constructeur : sélectionnez le <strong>type</strong>,
                définissez les <strong>points</strong>, rédigez l'
                <strong>intitulé</strong> et les réponses, puis cliquez{" "}
                <strong>Ajouter</strong>.
              </li>
              <li>
                ③ L'exercice est créé en état <strong>Brouillon</strong> —
                programmez-le ensuite pour le diffuser.
              </li>
            </ul>
          </div>
