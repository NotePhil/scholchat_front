...
const EditExerciseForm = ({
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const toAdd = newMatiereIds.filter(
        (id) => !originalMatiereIds.includes(id),
      );
      const toRemove = originalMatiereIds.filter(
        (id) => !newMatiereIds.includes(id),
      );
      await Promise.allSettled([
        ...toAdd.map((id) =>
          exerciseService.lierExerciseAMatiere(exerciseId, id),
        ),
        ...toRemove.map((id) =>
          exerciseService.delierExerciseDeMatiere(exerciseId, id),
        ),
      ]);

      // 3. Delete removed questions
      await Promise.allSettled(
        removedIds.map((id) => questionReponseService.deleteQuestion(id)),
      );
      ...
      const questionResults = await Promise.allSettled(
        questions.map((q) =>
          q.id
            ? questionReponseService.updateQuestion(
                q.id,
                buildQuestionPayload(q),
              )
            : questionReponseService.createQuestion(
                exerciseId,
                buildQuestionPayload(q),
              ),
        ),
      );
      const failedQuestions = questionResults.filter(
        (r) => r.status === "rejected",
      );
      if (failedQuestions.length > 0) {
        failedQuestions.forEach((r) =>
          console.error(
            "[EditExerciseForm] failed to save question:",
            r.reason,
          ),
        );
