        ...
        {(type === "REPONSE_COURTE" ||
          ...
          <div>
            ...
            <div className="p-3 bg-gray-50 rounded border-l-4 border-blue-400">
              <Text>
                {question.reponse || <Text type="secondary">Non définie</Text>}
              </Text>
            </div>
          </div>
        )}
            ...
            <Tooltip title={`Bonne(s) réponse(s): ${correct}`}>
              <Space size={2} wrap>
                {sorted.slice(0, 3).map((c) => (
                  <Tag
                    key={c.id}
                    color={c.estCorrect ? "green" : "default"}
                    className="text-xs"
                  >
                    {c.texte}
                  </Tag>
