...
const ExerciseInfoPanel = ({ exercise, editing, form }) => {
    ...
    return (
      <Form form={form} layout="vertical" className="pt-2">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-6">
          ...
          >
            <Input prefix={<FontAwesomeIcon icon={faBook} />} />
          </Form.Item>
          <Form.Item
            name="niveau"
            label="Niveau"
  ...
  return (
    <div>
      ...
      <InfoRow label="Visibilité">
        {restrictionTag(exercise.restriction)}
      </InfoRow>
      <InfoRow label="Créé le">
        <span className="flex items-center gap-1 text-gray-500">
          <FontAwesomeIcon icon={faCalendarDays} className="text-xs" />{" "}
          {fmtDate(exercise.dateCreation)}
        </span>
      </InfoRow>
        ...
        <InfoRow label="Matières">
          <div className="flex flex-wrap gap-1">
            {exercise.matieres.map((m) => (
              <Tag key={m.id} color="purple" className="text-xs m-0">
                {m.nom}
              </Tag>
            ))}
          </div>
      ...
      <InfoRow label="Description">
        <span className="text-gray-600 leading-relaxed">
          <FontAwesomeIcon icon={faFileLines} className="mr-1 text-gray-400" />
