...
import { Select, Spin, Empty, Tag, Button, Input, Typography, Space } from "antd";
import {
  UserOutlined, CheckCircleOutlined, ClockCircleOutlined,
  CloseCircleOutlined, SaveOutlined, TrophyOutlined,
...
const ETAT_COLORS = {
  EN_COURS:              { color: "#d48806", bg: "#fffbe6", label: "En cours" },
  SOUMIS:                { color: "#1677ff", bg: "#e6f4ff", label: "Soumis" },
  EN_ATTENTE_CORRECTION: { color: "#d46b08", bg: "#fff7e6", label: "En attente" },
  CORRIGE:               { color: "#531dab", bg: "#f9f0ff", label: "Corrigé" },
  VALIDE:                { color: "#389e0d", bg: "#f6ffed", label: "Validé" },
};

const EtatBadge = ({ etat }) => {
  const c = ETAT_COLORS[etat] || { color: "#8c8c8c", bg: "#fafafa", label: etat };
  return (
...
const CorrectionsPanel = ({ exerciseId, programmations }) => {
  ...
  const loadData = async () => {
    ...
    try {
      const [q, a, p] = await Promise.all([
        ...
        participationExerciseService.getParticipationsByExercise(selectedProgId),
      ]);
      setQuestions(q || []);
      setAnswers(a || []);
      setParticipations(p || []);
  ...
  const handleGradeChange = (studentId, questionId, field, value) => {
    setGradeInputs(prev => ({
      ...prev,
      [`${studentId}-${questionId}`]: { ...prev[`${studentId}-${questionId}`], [field]: value },
    }));
  };
    ...
    <div>
      ...
      {programmations.length > 1 && (
        <div className="mb-4 flex items-center gap-3">
          <Text className="text-sm text-gray-500 flex-shrink-0">Programmation :</Text>
          <Select value={selectedProgId} onChange={setSelectedProgId} style={{ minWidth: 260 }} size="middle">
            {programmations.map(p => (
              <Option key={p.id} value={p.id}>
                {p.typeAssignation === "DEVOIR" ? "📋 Devoir" : "📝 Exercice"} —{" "}
                {p.dateExoPrevue ? new Date(p.dateExoPrevue).toLocaleDateString("fr-FR") : p.id.slice(0, 8)}
                {p.classesDiffusees?.length > 0 && ` (${p.classesDiffusees.map(c => c.nom).join(", ")})`}
              </Option>
            ))}
          </Select>
        </div>
      ...
      <div className="grid grid-cols-3 gap-3 mb-5">
        {[
          { label: "Participants", value: students.length, color: "#1677ff", bg: "#e6f4ff" },
          { label: "Ont répondu", value: students.filter(s => s.answers.length > 0).length, color: "#389e0d", bg: "#f6ffed" },
          { label: "En attente", value: participations.filter(p => p.etatSoumission === "EN_ATTENTE_CORRECTION").length, color: "#d46b08", bg: "#fff7e6" },
      ...
      {loading ? (
        ...
        <Empty image={Empty.PRESENTED_IMAGE_SIMPLE} description="Aucune soumission pour le moment" />
      ) : (
              ...
              <div key={student.id} className="rounded-xl border overflow-hidden bg-white"
                ...
                >
                  ...
                  <div className="flex items-center gap-3 flex-shrink-0">
                    ...
                    <span className="text-xs text-gray-400">{student.answers.length}/{questions.length} rép.</span>
                ...
                {isExpanded && (
                  <div className="border-t bg-gray-50 p-4 space-y-3">
                    {/* Overall grade */}
                    <div className="p-3 bg-white rounded-lg border" style={{ borderColor: "#e8edf5" }}>
                      <Text strong className="text-sm block mb-2">
                        <TrophyOutlined className="mr-1 text-amber-500" />Note globale
                      </Text>
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                          {savedKeys.has(`overall-${student.id}`) ? "Sauvegardé ✓" : "Sauvegarder"}
                        </Button>
                      </div>
                    ...
                    {questions.map((q, idx) => {
                      ...
                      return (
                        <div key={q.id} className={`p-3 rounded-lg border ${answer?.estCorrecte === true ? "bg-green-50 border-green-200" : answer?.estCorrecte === false ? "bg-red-50 border-red-200" : "bg-white border-gray-200"}`}>
                          <div className="flex items-start gap-2 mb-2">
                            ...
                            {answer?.estCorrecte === true && <CheckCircleOutlined style={{ color: "#52c41a", fontSize: 16 }} />}
                            {answer?.estCorrecte === false && <CloseCircleOutlined style={{ color: "#ff4d4f", fontSize: 16 }} />}
                            {needsManual && <ClockCircleOutlined style={{ color: "#fa8c16", fontSize: 16 }} />}
                          </div>
                          ...
                          {answer ? (
                            <div className="ml-8 space-y-1">
                              ...
                              {needsManual && (
                                <div className="mt-2 p-2 rounded-lg" style={{ background: "#fff7e6", border: "1px solid #ffd591" }}>
                                  <Text className="text-xs font-medium block mb-2" style={{ color: "#d46b08" }}>
                                  ...
                                  <div className="flex flex-wrap items-center gap-2">
                                    <Select
                                      ...
                                      onChange={v => handleGradeChange(student.id, q.id, "estCorrecte", v)}
                                      style={{ width: 110 }}
                                    >
                                    ...
                                    <Input
                                      ...
                                      onChange={e => handleGradeChange(student.id, q.id, "note", e.target.value)}
                                      style={{ width: 70 }}
                                    />
                                    <Input
                                      size="small"
                                      ...
                                      onChange={e => handleGradeChange(student.id, q.id, "appreciation", e.target.value)}
                                    ...
                                    <Button
                                      ...
                                      onClick={() => handleSaveGrade(student.id, q.id)}
                                    >
                              ...
                              {answer.note && <Text type="secondary" className="text-xs">Note : {answer.note}</Text>}
                              {answer.appreciation && <Text type="secondary" className="text-xs italic">"{answer.appreciation}"</Text>}
                            </div>
                          ) : (
