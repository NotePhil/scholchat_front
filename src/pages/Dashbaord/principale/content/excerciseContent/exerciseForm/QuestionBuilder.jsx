        ...
        <div className="flex gap-3">
          {choix.map((c, i) => (
            <button key={i} type="button"
              onClick={() => onChange({ ...question, choixReponses: choix.map((x, j) => ({ ...x, estCorrect: j === i })) })}
  ...
  if (TYPES_WITH_CHOICES.includes(type)) {
    ...
    return (
      <div>
        ...
        <div className="space-y-2">
          {choix.map((c, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              />
              {choix.length > 2 && (
                <button type="button" onClick={() => removeChoix(i)}
        ...
        <button type="button" onClick={addChoix}
  ...
  if (TYPES_OPEN.includes(type)) {
    return (
      <div>
        ...
        {type === "REPONSE_COURTE"
          ? <Input value={question.reponse}
              onChange={e => onChange({ ...question, reponse: e.target.value })}
              placeholder="Ex: H₂O" style={{ borderRadius: 8 }} />
          : <TextArea rows={3} value={question.reponse}
              onChange={e => onChange({ ...question, reponse: e.target.value })}
...
const QuestionBuilder = ({ questions, currentQuestion, editingIndex, onChange, onAdd, onEdit, onRemove, onCancelEdit }) => {
  ...
  const handleTypeChange = (value) =>
    onChange({ ...currentQuestion, typeQuestion: value, choixReponses: getDefaultChoix(value), reponse: "" });

  return (
    <div>
      {/* Questions list */}
      {questions.length > 0 && (
        <div className="mb-4 rounded-xl border overflow-hidden" style={{ borderColor: "#ede9fe", background: "#faf9ff" }}>
          <div className="px-3 py-2 border-b text-xs font-semibold text-purple-700 uppercase tracking-wide"
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm font-medium text-gray-800 truncate">{q.intitule || <span className="text-gray-400 italic">Sans intitulé</span>}</div>
      ...
      {/* Builder */}
      <div className="rounded-xl border border-dashed p-4" style={{ borderColor: "#c4b5fd", background: "#faf9ff" }}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-3">
          <div className="sm:col-span-2">
            <label className="text-xs font-medium text-gray-600 block mb-1">Type de question</label>
          ...
          <div>
            ...
            <InputNumber min={0.5} max={100} step={0.5}
              ...
              onChange={v => onChange({ ...currentQuestion, points: v })}
              style={{ width: "100%" }} />
          </div>
        </div>
        ...
        {/* Intitulé */}
        <div className="mb-3">
          <label className="text-xs font-medium text-gray-600 block mb-1">Intitulé de la question</label>
          <TextArea rows={2}
            value={currentQuestion.intitule}
            onChange={e => onChange({ ...currentQuestion, intitule: e.target.value })}
        ...
        <div className="flex justify-end gap-2">
          {editingIndex !== null && (
            <Button onClick={onCancelEdit} style={{ borderRadius: 8 }}>Annuler</Button>
          )}
          <Button type="primary" icon={editingIndex !== null ? <SaveOutlined /> : <PlusOutlined />}
            onClick={onAdd}
