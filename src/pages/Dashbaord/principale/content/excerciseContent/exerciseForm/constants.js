...
export const QUESTION_TYPES = [
  ...
  },
  {
    ...
    desc: "Texte développé",
    icon: <FontAwesomeIcon icon={faFileLines} />,
  },
  {
    value: "DEVELOPPEMENT",
    label: "Développement",
    desc: "Rédaction libre",
    icon: <FontAwesomeIcon icon={faPenToSquare} />,
  },
  {
    value: "ASSOCIATION",
    label: "Association",
    desc: "Relier les éléments",
...
export const TYPES_WITH_CHOICES = [
  "QCM",
  "VRAI_FAUX",
  "ASSOCIATION",
  "CLASSEMENT",
...
export const validateQuestion = (q, messageApi) => {
  if (!q.intitule.trim()) {
    messageApi.warning("L'intitulé de la question est requis");
    return false;
  }
  const type = q.typeQuestion;
  if (type === "VRAI_FAUX") {
    if (!q.choixReponses.some((c) => c.estCorrect)) {
      messageApi.warning("Sélectionnez Vrai ou Faux");
      return false;
    }
  } else if (TYPES_WITH_CHOICES.includes(type)) {
    if (q.choixReponses.length < 2) {
      messageApi.warning("Au moins 2 choix requis");
      return false;
    }
    if (q.choixReponses.some((c) => !c.texte.trim())) {
      messageApi.warning("Tous les choix doivent avoir un texte");
      return false;
    }
    if (type === "QCM" && !q.choixReponses.some((c) => c.estCorrect)) {
      messageApi.warning("Cochez au moins une bonne réponse");
      return false;
    }
  } else if (TYPES_OPEN.includes(type)) {
    if (!q.reponse.trim()) {
      messageApi.warning("La réponse attendue est requise");
      return false;
    }
  }
  return true;
