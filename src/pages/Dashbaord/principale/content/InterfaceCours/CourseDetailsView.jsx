...
const processStorageUrls = async (html, redacteurId) => {
  ...
  for (const original of urlsToRefresh) {
    try {
      ...
      const downloadData =
        await minioS3Service.generateDownloadUrlByPath(relativePath);
    ...
    try {
      ...
      await Promise.all(
        Array.from(videos).map(async (video) => {
          ...
          const media = await minioS3Service.findMediaByFileName(
            captionText,
            redacteurId,
          );
          if (media?.id) {
...
const CourseDetailsView = ({ courseId, onBack }) => {
  ...
  const fetchCourseDetails = async () => {
    try {
      ...
      const formattedChapters = courseData.chapitres
            ...
            materials: [
              {
                ...
                size: chapitre.contenu
                  ? `${Math.ceil(chapitre.contenu.length / 1024)} KB`
                  : "0 KB",
                completed: false,
              },
            ],
      ...
      await Promise.all(
        formattedChapters.map(async (ch) => {
          if (ch.contenu) {
            try {
              const withProxyUrls = await processStorageUrls(
                ch.contenu,
                courseData.redacteurId,
              );
              processed[ch.id] = await processHtmlImages(
                withProxyUrls,
                blobUrlsRef.current,
              );
            } catch (e) {
  ...
  const extractFilesFromChapters = (chaptersData) => {
    try {
      ...
      (chaptersData || []).forEach((chapter, chapterIndex) => {
        if (chapter.contenu) {
          console.log(
            `Chapter ${chapterIndex} contenu preview:`,
            chapter.contenu.substring(0, 500),
          );
          ...
          images.forEach((img, imgIndex) => {
            ...
            const fileName =
              ...
              absoluteSrc.split("/").pop().split("?")[0] ||
              `Image_${chapterIndex}_${imgIndex}.jpg`;
            extractedFiles.push({
          ...
          videos.forEach((video, vidIndex) => {
            ...
            try {
              ...
              const name =
                ...
                `Video_${chapterIndex}_${vidIndex}.mp4`;
              extractedFiles.push({
          ...
          links.forEach((link, linkIndex) => {
            ...
            const fileName =
              ...
              `File_${chapterIndex}_${linkIndex}`;
            ...
            else if (href.includes("/media/") && href.includes("/content")) {
              contentType = "video/mp4";
              type = "video";
            }
            extractedFiles.push({
  ...
  const fetchMinioFiles = async (coursId) => {
    try {
      const { minioS3Service } =
        await import("../../../../../services/minioS3");
      ...
      if (Array.isArray(mediaFiles) && mediaFiles.length > 0) {
        const formatted = mediaFiles.map((media, index) => ({
          ...
          fileName:
            media.fileName || media.originalFileName || `Fichier_${index}`,
  ...
  const handlePlayVideo = (material) => {
    ...
    const file = courseFiles.find(
      (f) => f.fileName === material.titre || f.id === material.id,
    );
  ...
  const getFileIcon = (fileName, contentType) => {
    if (
      ...
      return <FontAwesomeIcon icon={faImage} className="w-4 h-4" />;
    }
    if (contentType?.includes("pdf") || fileName?.endsWith(".pdf")) {
      return <FontAwesomeIcon icon={faFileLines} className="w-4 h-4" />;
    }
    if (
      contentType?.startsWith("video/") ||
      fileName?.match(/\.(mp4|avi|mov|wmv|flv|webm)$/i)
    ) {
  ...
  const renderChapterContent = (content, chapterId) => {
    ...
    if (!html)
      return <p className="text-gray-500 italic">Aucun contenu disponible</p>;
    return (
  ...
  const getItemIcon = (type) => {
    switch (type) {
      ...
      default:
        return <FontAwesomeIcon icon={faBookOpen} className="w-4 h-4" />;
    }
  };
  ...
  return (
    <div>
          ...
          <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
            <div className="relative h-64 bg-gradient-to-r from-blue-600 to-purple-600">
              ...
              <div className="relative p-8 h-full flex flex-col justify-end">
                <div className="flex items-center gap-2 mb-4">
                  <span
                    className={`px-4 py-1.5 backdrop-blur-md text-white text-xs font-bold rounded-full border border-white/20 uppercase tracking-widest shadow-lg ${course?.restriction === "PUBLIC" ? "bg-emerald-500/40" : "bg-blue-500/30"}`}
                  >
                ...
                <div className="text-white transform transition-all duration-500 translate-y-0 group-hover:-translate-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-6 text-sm">
                    ...
                    <div className="flex items-center gap-2 px-3 py-1.5 bg-white/10 rounded-lg backdrop-blur-sm border border-white/10">
                      ...
                      />
                      <span className="font-semibold">
                        {courseFiles.length + minioFiles.length} fichiers
                        ressources
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex border-b">
              <button
                ...
                className={`px-6 py-4 font-medium transition-colors ${activeTab === "content" ? "text-blue-600 border-b-2 border-blue-600 bg-blue-50" : "text-gray-600 hover:text-gray-900"}`}
              >
                Contenu du cours
              </button>
            </div>
            ...
            {activeTab === "content" && (
              <div className="space-y-4">
                ...
                {chapters.map((chapter, index) => (
                  ...
                  >
                    <div
                      className={`p-6 cursor-pointer transition-all duration-200 ${chapter.locked ? "bg-gray-50" : "hover:bg-gray-50"}`}
                      onClick={() =>
                        !chapter.locked && toggleChapter(chapter.id)
                      }
                    >
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-4">
                          <div
                            className={`flex items-center justify-center w-12 h-12 rounded-xl font-bold text-white ${chapter.completed ? "bg-green-500" : chapter.locked ? "bg-gray-400" : "bg-blue-500"}`}
                          >
                            {index + 1}
                          </div>
                          <div>
                            <h3
                              className={`text-xl font-bold ${chapter.locked ? "text-gray-400" : "text-gray-900"}`}
                            >
                              Chapitre {index + 1}: {chapter.titre}
                            </h3>
                            {chapter.description && (
                              <p
                                className={`text-sm mt-1 ${chapter.locked ? "text-gray-400" : "text-gray-600"}`}
                              >
                                {chapter.description}
                              </p>
                            )}
                            <div className="flex items-center space-x-4 mt-2">
                              <span
                                className={`text-xs flex items-center ${chapter.locked ? "text-gray-400" : "text-gray-500"}`}
                              >
                                <FontAwesomeIcon
                                  icon={faFileLines}
                          ...
                          <div className="bg-white rounded-xl p-6 border border-green-100">
                            ...
                            {filesLoading ? (
                              ...
                              <div className="space-y-3">
                                {courseFiles.filter(
                                  (file) => file.chapterTitle === chapter.titre,
                                ).length > 0 ? (
                                  courseFiles
                                    .filter(
                                      (file) =>
                                        file.chapterTitle === chapter.titre,
                                    )
                                    .map((file, fileIndex) => (
                                      <div
                                      ...
                                      >
                                        <div className="flex items-center space-x-3 flex-1 min-w-0">
                                          <div
                                            className={`p-3 rounded-lg ${getFileTypeColor(file.fileName, file.contentType)}`}
                                          >
                                            {getFileIcon(
                                              file.fileName,
                                              file.contentType,
                                  ...
                                  <div className="text-center py-8 bg-gray-50 rounded-lg border-2 border-dashed border-gray-200">
                                    ...
                                    <p className="text-gray-500 text-sm">
                                      Aucune ressource disponible pour ce
                                      chapitre
                                    </p>
                                  </div>
                                )}
                  ...
                  <div className="bg-white rounded-xl shadow-lg p-6 border border-purple-100">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <span className="text-sm text-gray-500 bg-gray-100 px-3 py-1 rounded-full">
                        {minioFiles.length} fichier
                        {minioFiles.length !== 1 ? "s" : ""}
                      </span>
                    </div>

                    <p className="text-gray-600 text-sm mb-4 italic">
                      Fichiers supplémentaires fournis par le professeur pour
                      l'ensemble du cours
                    </p>
                      ...
                      {minioFiles.map((file, index) => (
                        ...
                        >
                          <div className="flex items-start space-x-3">
                            <div
                              className={`p-3 rounded-xl ${getFileTypeColor(file.fileName, file.contentType)} group-hover:scale-110 transition-transform`}
                            >
                              {getFileIcon(file.fileName, file.contentType)}
                            </div>
                            <div className="flex-1 min-w-0">
                              <h5
                                className="font-semibold text-gray-900 text-sm truncate pr-16"
                                title={file.fileName}
                              >
              ...
              <div className="bg-white rounded-xl shadow-lg p-8 border border-blue-100">
                ...
                <div className="prose max-w-none prose-blue">
                  <div className="bg-blue-50 p-6 rounded-2xl mb-8 border border-blue-100 italic text-blue-900 quote">
                    {course?.description ||
                      "Aucune description détaillée disponible pour ce cours."}
                  </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {[
                      ...
                      <div
                        key={i}
                        className="flex items-center space-x-3 p-3 bg-white border border-gray-100 rounded-xl"
                      >
                        <FontAwesomeIcon
                          icon={faCircleCheck}
            ...
            {activeTab === "discussions" && (
              <div className="bg-white rounded-xl shadow-lg p-6">
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-600">
                    Soyez le premier à démarrer une discussion sur ce cours.
                  </p>
                </div>
              </div>
            )}
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="font-bold text-green-600 bg-green-50 px-3 py-1 rounded-full text-sm">
                    {progression.chapitresCompletes ?? 0}/
                    {progression.totalChapitres ?? chapters.length}
                  </span>
                </div>
                <div className="flex items-center justify-between">
            ...
            <div className="bg-blue-600 rounded-xl shadow-lg p-6 text-white overflow-hidden relative">
              ...
              <p className="text-blue-100 text-sm mb-4">
                Contactez votre professeur pour toute question sur le contenu.
              </p>
      ...
      {toast.show && (
        <div
          className={`fixed bottom-8 right-8 z-[1000] px-6 py-4 rounded-2xl shadow-2xl flex items-center space-x-3 animate-in fade-in slide-in-from-bottom-5 duration-300 ${toast.type === "success" ? "bg-green-600" : toast.type === "error" ? "bg-red-600" : "bg-blue-600"} text-white`}
        >
          {toast.type === "success" ? (
