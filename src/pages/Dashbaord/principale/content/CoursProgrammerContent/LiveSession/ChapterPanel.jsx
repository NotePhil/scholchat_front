...
const getFileType = (url = "", contentType = "") => {
  ...
  if (
    ...
    /\.(jpg|jpeg|png|gif|webp|svg)$/.test(u)
  )
    return "image";
  if (contentType.startsWith("video/") || /\.(mp4|webm|avi|mov|mkv)$/.test(u))
    return "video";
  ...
  if (/\.(ppt|pptx)$/.test(u) || contentType.includes("presentation"))
    return "office";
...
const ImagePopup = ({ url, fileName, onClose }) => {
  ...
  return (
    ...
    >
      <div
        className="flex items-center justify-between px-3 py-2 bg-black/70 flex-shrink-0"
...
const FileViewer = ({
  ...
  const canPreview =
    type === "image" || type === "video" || type === "pdf" || type === "office";
  if (type === "image") {
    return (
      <div className="rounded-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div className="flex items-center gap-3 px-3 py-2.5 bg-gray-50 dark:bg-gray-800">
          <span
            className={`text-[10px] font-bold px-1.5 py-0.5 rounded ${meta.color} flex-shrink-0`}
          >
          ...
          >
            <FontAwesomeIcon icon={faEye} className="w-3 h-3" /> Voir
          </button>
          <a
            href={url}
            download={fileName}
            target="_blank"
            rel="noreferrer"
            className="p-1.5 rounded-lg bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 text-gray-700 dark:text-gray-300"
          >
          ...
          {(type === "pdf" || type === "office") && (
            <div className="relative">
              ...
              <div className="flex items-center gap-2 px-3 py-1.5 bg-gray-50 border-b border-gray-200 dark:bg-gray-800 dark:border-gray-700">
                ...
                <button
                  onClick={() => setIframeZoom((z) => Math.max(0.5, z - 0.25))}
                ...
                <span className="text-xs w-10 text-center text-gray-600">
                  {Math.round(iframeZoom * 100)}%
                </span>
                <button
                  onClick={() => setIframeZoom((z) => Math.min(3, z + 0.25))}
                  className="p-1 bg-gray-200 hover:bg-gray-300 rounded text-gray-700"
                >
                  <FontAwesomeIcon
                    icon={faMagnifyingGlassPlus}
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="flex items-center gap-1 px-2 py-1 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 rounded text-xs"
                >
                  <FontAwesomeIcon
...
const refreshContentUrls = async (html, redacteurId) => {
  ...
  const imgPromises = Array.from(root.querySelectorAll("img")).map(
    async (img) => {
      const src = img.getAttribute("src");
      if (isMinioUrl(src)) img.setAttribute("src", await refreshUrl(src));
    },
  );
  const aPromises = Array.from(root.querySelectorAll("a[href]")).map(
    async (a) => {
      const href = a.getAttribute("href");
  ...
  if (redacteurId) {
    ...
    await Promise.all(
      blobVideos.map(async (video) => {
        const container = video.parentElement;
        const caption = container?.querySelector("div");
        const captionText = caption?.textContent?.trim() || "";
        console.log(
          "[ChapterPanel] Resolving blob video, caption:",
          captionText,
          "redacteurId:",
          redacteurId,
        );
        ...
        try {
          ...
          console.warn("[ChapterPanel] blob resolve error:", e);
        }
      }),
    );
  }
...
const ChapterPanel = ({
  ...
  const isCompleted = (chapId) =>
    progress.some((p) => p.chapitreId === chapId && p.completed);
  ...
  useEffect(() => {
    ...
    const process = async () => {
      const refreshed = await refreshContentUrls(
        currentChap.contenu || "",
        redacteurId,
      );
    ...
    if (anchor) {
      ...
      if (href && href.startsWith("http")) {
        ...
        if (t === "image") {
          setImagePopup({
            url: href,
            fileName: anchor.textContent.trim() || "Image",
          });
        } else {
          setFileViewer({
            url: href,
    ...
    if (fileDiv) {
      ...
      if (innerAnchor) {
        ...
        if (t === "image")
          setImagePopup({
            url: href,
            fileName: innerAnchor.textContent.trim() || "Image",
          });
        else
          setFileViewer({
            url: href,
            fileName: innerAnchor.textContent.trim() || "Fichier",
            contentType: "",
          });
        return;
      }
            ...
            {chapitres.map((chap, idx) => {
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center gap-2.5 px-3 py-2 text-left transition-all border-b border-gray-100 dark:border-gray-700 last:border-0 ${active ? "bg-indigo-50 dark:bg-indigo-900/30 border-l-4 border-l-indigo-500" : done ? "hover:bg-gray-50 dark:hover:bg-gray-700/50" : isModerator ? "hover:bg-gray-50 dark:hover:bg-gray-700/50" : "opacity-50 cursor-not-allowed"}`}
                >
                  <div
                    className={`w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[10px] font-bold ${done ? "bg-green-100 text-green-600" : active ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-500"}`}
                  >
                    {done ? (
                      <FontAwesomeIcon
                        icon={faCircleCheck}
                  ...
                  <div className="flex-1 min-w-0">
                    <span
                      className={`text-xs truncate block ${active ? "font-semibold text-indigo-700 dark:text-indigo-300" : "text-gray-700 dark:text-gray-300"}`}
                    >
                      {chap.titre}
                    </span>
                    {chapMedia.length > 0 && (
          ...
          {currentChap ? (
            <div>
              ...
              >
                ...
                />
                <h3 className="font-bold text-gray-900 dark:text-white text-sm flex-1 truncate">
                  {currentChap.titre}
                </h3>
                {contentExpanded ? (
                  <FontAwesomeIcon
              ...
              {contentExpanded && (
                <div className="p-4 space-y-4">
                  ...
                  {mediaItems.filter(
                    (m) => getFileType(m.url, m.contentType) !== "image",
                  ).length > 0 && (
            ...
            <div className="flex flex-col items-center justify-center h-full text-center py-8 px-4">
              ...
              {isModerator && (
                <p className="text-xs text-gray-400 mt-1">
                  Cliquez sur un chapitre pour le partager avec les étudiants
                </p>
              )}
            </div>
          )}
        ...
        {fileViewer && (
          <div className="absolute inset-0 z-50 bg-white dark:bg-gray-900 flex flex-col">
            ...
            <div className="flex-1 overflow-hidden">
              <iframe
                src={
                  getFileType(fileViewer.url, fileViewer.contentType) === "pdf"
