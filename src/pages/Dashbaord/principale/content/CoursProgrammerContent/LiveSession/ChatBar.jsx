...
import { Send, Hand, Users, MessageCircle, X } from "lucide-react";

const ChatBar = ({ messages = [], participants = [], onSendMessage, onRaiseHand, handRaises = [] }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700">
      ...
      <div className="flex border-b border-gray-200 dark:border-gray-700">
        {[
          { key: "chat", icon: MessageCircle, label: "Chat", badge: messages.length },
          { key: "participants", icon: Users, label: `Participants (${participants.length})` },
          { key: "hands", icon: Hand, label: "Mains levées", badge: handRaises.length },
      ...
      <div className="flex-1 overflow-y-auto min-h-0">
              ...
              return (
                <div key={i} className={`flex gap-2 ${isMe ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`max-w-[75%] ${isMe ? "items-end" : "items-start"} flex flex-col`}>
                    {!isMe && <span className="text-[10px] text-gray-400 mb-0.5">{msg.userName}</span>}
                    <div className={`px-3 py-1.5 rounded-2xl text-xs ${
                      isMe ? "bg-indigo-600 text-white" : "bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-200"
                    }`}>
                      {msg.message}
                    </div>
        ...
        {tab === "participants" && (
          <div className="p-3 space-y-1.5">
            {participants.length === 0 && (
              <p className="text-center text-xs text-gray-400 py-4">Aucun participant</p>
            )}
            {participants.map((p, i) => (
              <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <span className="text-xs text-gray-700 dark:text-gray-300">{p.userName}</span>
                {p.userId === currentUserId && (
                  <span className="ml-auto text-[9px] bg-indigo-100 text-indigo-600 px-1.5 py-0.5 rounded-full">Vous</span>
                )}
              </div>
            ))}
          </div>
        ...
        {tab === "hands" && (
          <div className="p-3 space-y-1.5">
            {handRaises.length === 0 && (
              <p className="text-center text-xs text-gray-400 py-4">Aucune main levée</p>
            )}
            {handRaises.map((h, i) => (
              <div key={i} className="flex items-center gap-2 px-2 py-1.5 rounded-lg bg-amber-50 dark:bg-amber-900/20">
                <Hand className="w-4 h-4 text-amber-500 flex-shrink-0" />
                <span className="text-xs text-gray-700 dark:text-gray-300">{h.userName}</span>
              </div>
            ))}
          </div>
        )}
