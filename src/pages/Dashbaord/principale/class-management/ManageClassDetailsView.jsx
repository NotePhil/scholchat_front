      ...
      >
        {tabs.map((tab) => (
          <button
            ...
            className={`flex-shrink-0 flex items-center gap-1.5 py-3 px-4 text-sm font-medium border-b-2 transition-all whitespace-nowrap ${activeTab === tab.key ? "border-indigo-600 text-indigo-700" : "border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300"}`}
          >
            {tab.icon}
            {tab.label}
          </button>
...
const ManageClassDetailsView = ({
  ...
  const [classRejectMotifsLoading, setClassRejectMotifsLoading] =
    useState(false);
    ...
    try {
      ...
      const enrichedDetails = {
        ...
        droitPublication:
          ...
          "PROFESSEURS_SEULEMENT",
        etablissement:
          details.etablissement ||
          (details.etablissement_id
            ? {
                id: details.etablissement_id,
              }
            : null),
      ...
      if (rightsResult.success) {
        ...
        if (rightsUsers.length > 0) {
          setUsers((prevUsers) => {
            ...
            console.log(
              "Existing user IDs before merge:",
            ...
            rightsUsers.forEach((user) => {
              ...
              if (!existingIds.has(user.id)) {
                ...
                if (userType === "PROFESSEUR" || userType === "PROFESSOR") {
                  ...
                  categorized.eleves.push({
                    ...user,
                    typeUtilisateur: "ELEVE",
                  });
                } else if (userType === "PARENT") {
                  categorized.parents.push({
                    ...user,
                    typeUtilisateur: "PARENT",
                  });
                } else {
                  categorized.utilisateurs.push({
                    ...user,
            ...
            if (
              categorized.professeurs.length > 0 ||
              categorized.eleves.length > 0 ||
              categorized.parents.length > 0 ||
              categorized.utilisateurs.length > 0
            ) {
              const newState = {
                professeurs: [
                  ...prevUsers.professeurs,
                  ...categorized.professeurs,
                ],
                eleves: [...prevUsers.eleves, ...categorized.eleves],
                parents: [...prevUsers.parents, ...categorized.parents],
                utilisateurs: [
                  ...prevUsers.utilisateurs,
                  ...categorized.utilisateurs,
                ],
  ...
  const loadUsersWithAccess = async () => {
    try {
      ...
      const usersWithAccess =
        await AccederService.obtenirUtilisateursAvecAcces(classId);
      ...
      if (!usersWithAccess || usersWithAccess.length === 0) {
        console.log(
          "No users with access found for this class - will check publication rights",
        );
      ...
      const enrich = (users, pool) => {
        ...
        return users.map((u) => ({
          ...(safePool.find((p) => p.id === u.id) || {}),
          ...u,
        }));
      };
      categorizedUsers.professeurs = enrich(
        categorizedUsers.professeurs,
        allProfessors,
      );
  ...
  const loadCourses = async () => {
    try {
      ...
      const sorted = (data || []).sort((a, b) => {
        ...
        return (
          (order[a.etatCoursProgramme] ?? 99) -
          (order[b.etatCoursProgramme] ?? 99)
        );
      });
      setCourses(sorted);
    } catch (error) {
  ...
  const loadExercises = async () => {
    try {
      const data =
        await exerciseProgrammerService.getExercisesProgrammesParClasse(
          classId,
        );
      ...
      const sorted = (data || []).sort((a, b) => {
        ...
        return (
          (order[a.etat] ?? order[a.etatExercise] ?? 99) -
          (order[b.etat] ?? order[b.etatExercise] ?? 99)
        );
      });
      setExercises(sorted);
    } catch (error) {
  ...
  const loadEvents = async () => {
    try {
      ...
      const classEvents = allEvents.filter(
        (event) => event.classesIds && event.classesIds.includes(classId),
      );
  ...
  const handleTogglePublicationRights = async (professor, grant) => {
    try {
      if (grant) {
        ...
        message.success(
          `Droit de publication accordé à ${professor.prenom} ${professor.nom}`,
        );
      } else {
        await PublicationRightsService.removePublicationRights(
          professor.id,
          classId,
        );
        message.success(
          `Droit de publication retiré à ${professor.prenom} ${professor.nom}`,
        );
      }
      ...
      message.error(
        err.message ||
          "Erreur lors de la mise à jour des droits de publication",
      );
    }
  };
  ...
  const handleApproveByEstablishment = async (classeId, etablissementId) => {
    try {
      setActionLoading("approve");
      await classService.approuverClasseParEtablissement(
        classeId,
        etablissementId,
      );
  ...
  const handleRejectByEstablishment = async (classeId, etablissementId) => {
    try {
      ...
      await classService.rejeterClasseParEtablissement(
        classeId,
        etablissementId,
      );
      message.success("Classe rejetée avec succès");
  ...
  const handleDelete = async () => {
    ...
    if (
      isProfessor &&
      !isAdmin &&
      moderatorsWithRights.length === 1 &&
      moderatorsWithRights[0].id === currentUserId
    ) {
      message.error(
  ...
  const getStatusTag = (etat) => {
    ...
    const config = statusConfig[etat] || {
      color: "default",
      text: etat,
    };
  ...
  const getPublicationRightsTag = (droit) => {
    const rightsConfig = {
      TOUS: {
        color: "green",
        text: "Tous peuvent publier",
      },
      MODERATEUR_SEULEMENT: {
        color: "blue",
        text: "Modérateur seulement",
      },
      PARENTS_ET_MODERATEUR: {
        color: "orange",
        text: "Parents et modérateur",
      },
      PROFESSEURS_SEULEMENT: {
        color: "blue",
        text: "Professeurs seulement",
      },
    };
    const config = rightsConfig[droit] || {
      color: "default",
      text: droit,
    };
    return <Tag color={config.color}>{config.text}</Tag>;
  ...
  const renderActionButtons = () => {
    ...
    const isProfessor =
      ...
      userRole === "PROFESSOR" ||
      userRole?.includes("PROFESSOR");
    const establishmentEnforcesValidation =
      classDetails.etablissement?.optionEnvoiMailNewClasse === true;

    // Hide buttons if professor and establishment requires validation
    const shouldShowApprovalButtons =
      isPending &&
      !canSelfManage &&
      (isAdmin || (isProfessor && !establishmentEnforcesValidation));
    ...
    console.log(
      "Should Show Admin Buttons:",
      isAdmin && isPending && !canSelfManage,
    );
    console.log("==============================");
    return (
      <Card
      ...
      <div className="w-full">
            ...
            <div className="flex items-start gap-3 mb-3">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  {(() => {
                    const isCreator =
                      classDetails?.creatorId === currentUserId ||
          ...
          <div className="px-3 sm:px-6 pt-3">
            <div className="bg-white border border-slate-100 rounded-2xl shadow-sm p-4 flex items-center gap-3 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {onNavigateToEvents && (
                  <button
                    onClick={() => onNavigateToEvents(classId)}
                    className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold text-white transition-all hover:opacity-90 shadow-md"
                    style={{
                      background: "linear-gradient(135deg,#f6a623,#e07b00)",
                    }}
                  >
                    <FontAwesomeIcon
                      icon={faCalendarDays}
        ...
        <TabScrollBar
          ...
          tabs={[
            ...
            {
              key: "professeurs",
              label: `Professeurs (${users.professeurs.length})`,
              icon: <FontAwesomeIcon icon={faUser} />,
            },
            {
              key: "eleves",
              label: `Élèves (${users.eleves.length})`,
              icon: <FontAwesomeIcon icon={faUserGroup} />,
            },
            {
              key: "parents",
              label: `Parents (${users.parents.length})`,
              icon: <FontAwesomeIcon icon={faUserGroup} />,
            },
            {
              key: "utilisateurs",
            ...
            <div className="flex flex-col lg:flex-row gap-4 sm:gap-6">
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  <div className="px-4 py-3 space-y-3">
                    {[
                      {
                        label: "Nom",
                        value: classDetails.nom,
                      },
                      {
                        label: "Niveau",
                        value: classDetails.niveau,
                      },
                      {
                        label: "Date de création",
                        value: classDetails.dateCreation
                          ? new Date(
              ...
              <div className="flex-1 min-w-0">
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                  ...
                  <div className="px-4 py-4">{renderModeratorInfo()}</div>
                </div>
              </div>
            </div>
          )}
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                try {
                                  ...
                                  if (request?.id) {
                                    await AccederService.validerDemandeAcces(
                                      request.id,
                                    );
                                  } else {
                                    const reqs =
                                      await AccederService.obtenirDemandesAccesPourClasse(
                                        classId,
                                      );
                                    const r = reqs.find(
                                      (r) =>
                                        r.utilisateurId === u.id &&
                                        r.etat === "EN_ATTENTE",
                                    );
                                    if (r)
                                      await AccederService.validerDemandeAcces(
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <Table
                ...
                columns={[
                  {
                    title: "Cours",
                    dataIndex: ["cours", "titre"],
                    key: "titre",
                    render: (v, r) => v || r.description || "—",
                  },
                  {
                    title: "Date prévue",
                    dataIndex: "dateCoursPrevue",
                    key: "date",
                    render: (d) =>
                ...
                locale={{
                  emptyText: "Aucun cours programmé pour cette classe",
                }}
              />
            </div>
          )}
            ...
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                <div className="flex flex-col">
                  ...
                  <Text type="secondary" className="text-xs">
                    Liste des exercices pour cette classe
                  </Text>
                </div>
              ...
              <Table
                ...
                locale={{
                  emptyText: "Aucun exercice programmé pour cette classe",
                }}
              />
            </div>
          )}
          ...
          {activeTab === "events" && (
            <div className="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
              ...
              <Table
                ...
                locale={{
                  emptyText: "Aucun événement pour cette classe",
                }}
              />
            </div>
          )}
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
                ...
                {classRejectMotifsLoading ? (
                  ...
                  <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
                    {classRejectMotifs.map((m) => (
                      <label
                        key={m.id}
                        className={`flex items-start gap-2.5 p-2.5 rounded-xl border cursor-pointer transition-all ${classRejectSelectedCode === m.code ? "border-red-400 bg-red-50" : "border-slate-200 hover:border-slate-300 hover:bg-slate-50"}`}
                      >
                        <input
                          type="radio"
                          name="classMotif"
            ...
            <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-end gap-2 bg-slate-50/50">
              ...
              <button
                ...
                disabled={
                  !classRejectSelectedCode || actionLoading === "admin-reject"
                }
