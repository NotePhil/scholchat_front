...
const OffreInfoPanel = ({ type, entityId, isDark }) => {
  ...
  const handleAssignerParAdmin = async () => {
    ...
    try {
      ...
      if (type === "ETABLISSEMENT")
        await contratService.assignerOffreEtablissementParAdmin(
          entityId,
          payload,
        );
  ...
  const offreCible =
    offresDisponibles.find((o) => o.id === nouvelleOffreId) || null;
  const montant =
    (showAction === "changer" || showAction === "admin") && offreCible
      ? Number(
          periodicite === PeriodiciteContrat.ANNUEL
            ? offreCible.prixAnnuel
            : offreCible.prixMensuel,
        ) || 0
      : contrat
        ? Number(
            periodicite === PeriodiciteContrat.ANNUEL
        ...
        : 0;
  const offreLabelPaiement =
    showAction === "changer" || showAction === "admin"
      ? offreCible?.nom || "Nouvelle offre"
      : contrat?.offreNom || "Offre";
  const handlePaymentSuccess = async (paymentInfo) => {
    ...
    try {
      ...
      if (type === "ETABLISSEMENT") {
        if (showAction === "changer")
          await contratService.changerOffreEtablissement(entityId, payload);
        else
          await contratService.prolongerContratEtablissement(entityId, payload);
  ...
  if (loading)
    return (
      <div
        className={`rounded-2xl border p-4 ${cardBg} ${textMuted} text-sm animate-pulse`}
      >
  ...
  return (
    <div className={`rounded-2xl border p-4 sm:p-5 ${cardBg}`}>
      <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
        <h3 className={`font-semibold text-sm sm:text-base ${textMain}`}>
          Offre / Forfait actuel
        </h3>
        {contrat && (
          <span
            className={`text-xs px-2 py-0.5 rounded-full font-medium ${estActif ? "bg-green-100 text-green-700" : estExpire ? "bg-red-100 text-red-700" : "bg-gray-200 text-gray-600"}`}
          >
            {contrat.statut}
          </span>
        )}
      ...
      {!contrat && !error && (
        <p className={`text-sm ${textMuted}`}>
          Aucune offre associée pour le moment.
        </p>
      )}
      ...
      {contrat && (
        <div className={`text-sm space-y-1.5 ${textMuted}`}>
          <div className="flex justify-between gap-2">
            <span>Offre</span>
            <span className={`font-medium text-right ${textMain}`}>
              {contrat.offreNom}
            </span>
          </div>
          <div className="flex justify-between gap-2">
            ...
            <span className={`font-medium ${textMain}`}>
              {contrat.periodicite === "MENSUEL" ? "Mensuelle" : "Annuelle"}
            </span>
          </div>
          <div className="flex justify-between gap-2">
            <span>Montant</span>
            <span className={`font-medium ${textMain}`}>
              {Number(contrat.prixPaye || 0).toLocaleString("fr-FR")} FCFA
            </span>
          </div>
          {contrat.dateFin && (
            <div className="flex justify-between gap-2">
          ...
          {contrat.classesMax != null && (
            <div className="flex justify-between gap-2">
              <span>Classes</span>
              <span
                className={`font-medium ${contrat.classesUtilisees >= contrat.classesMax ? "text-red-600" : textMain}`}
              >
          ...
          {contrat.messagerie != null && (
            <div className="flex justify-between gap-2">
              ...
              <span className={`font-medium ${textMain}`}>
                {contrat.messagerie ? "Incluse" : "Non incluse"}
              </span>
            </div>
          )}
        </div>
      ...
      {contrat?.suppressionImminente && contrat.dateSuppressionPrevue && (
        <div className="mt-3 flex items-start gap-2 text-sm text-red-800 bg-red-100 border border-red-300 rounded-lg p-2.5">
          ...
          <span>
            ...
            <strong>
              {new Date(contrat.dateSuppressionPrevue).toLocaleString("fr-FR")}
            </strong>{" "}
            si l'offre n'est pas renouvelée.
          </span>
        </div>
      )}
      ...
      {estExpire && (
        <div className="mt-3 flex items-center gap-2 text-sm text-red-700 bg-red-50 border border-red-200 rounded-lg p-2.5">
          ...
          />{" "}
          Offre expirée — renouvelez pour réactiver.
        </div>
      )}
      ...
      {contrat && (
        <div className="flex flex-col sm:flex-row gap-2 mt-4">
          <button
            onClick={() => ouvrirAction("prolonger")}
          ...
          <button
            onClick={() => ouvrirAction("changer")}
            className={`flex items-center justify-center gap-1.5 px-3 py-2.5 sm:py-2 text-sm rounded-xl border font-medium active:scale-[0.98] transition-all ${isDark ? "border-gray-600 text-gray-200 hover:bg-gray-700" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}
          >
            <FontAwesomeIcon
      ...
      {isAdmin && (
        <button
          onClick={() => ouvrirAction("admin")}
          className={`flex items-center justify-center gap-1.5 px-3 py-2.5 sm:py-2 text-sm rounded-xl border border-dashed font-medium mt-2 w-full active:scale-[0.98] transition-all ${isDark ? "border-purple-500 text-purple-300 hover:bg-purple-500/10" : "border-purple-400 text-purple-700 hover:bg-purple-50"}`}
        >
          <FontAwesomeIcon
            icon={faWrench}
            style={{
          ...
          />{" "}
          Attribuer une offre (admin, sans paiement)
        </button>
      )}
      ...
      {showExpiredPopup && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm">
          <div
            className={`w-full sm:max-w-sm max-h-[85vh] overflow-y-auto ${modalBg} rounded-t-3xl sm:rounded-2xl shadow-2xl p-6 text-center`}
          >
            ...
            <p className={`text-sm ${textMuted} mb-5`}>
              Le forfait de{" "}
              {type === "ETABLISSEMENT" ? "cet établissement" : "cette classe"}{" "}
              a expiré
              {contrat?.offreNom ? ` (${contrat.offreNom})` : ""}. Renouvelez-le
      ...
      {showAction && !showPayment && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm">
          <div
            className={`w-full sm:max-w-sm max-h-[92vh] overflow-y-auto ${modalBg} rounded-t-3xl sm:rounded-2xl shadow-2xl p-5 sm:p-6`}
          >
            <div className="sm:hidden flex justify-center -mt-1 mb-2">
              <div
                className={`h-1.5 w-10 rounded-full ${isDark ? "bg-gray-600" : "bg-gray-300"}`}
              />
            </div>
            ...
            {showAction === "admin" && (
              <div className="mb-4 flex items-center gap-2 text-xs text-purple-700 bg-purple-50 border border-purple-200 rounded-xl p-2.5">
                ...
                />{" "}
                Attribution directe, sans passer par le paiement — réservé aux
                admins.
              </div>
            )}
            ...
            {(showAction === "changer" || showAction === "admin") && (
              <div className="mb-4">
                <label
                  className={`block text-sm font-medium ${textMuted} mb-1.5`}
                >
                  {showAction === "admin"
                    ? "Offre à attribuer"
                    : "Nouvelle offre"}
              ...
              <div className="flex gap-2">
                {[
                  ...
                  <button
                    key={val}
                    type="button"
                    onClick={() => setPeriodicite(val)}
                    className={`flex-1 px-3 py-2.5 rounded-xl border text-sm font-medium transition-colors ${periodicite === val ? "bg-blue-600 border-blue-600 text-white" : isDark ? "border-gray-600 text-gray-300 hover:bg-gray-700" : "border-gray-300 text-gray-700 hover:bg-gray-50"}`}
                  >
                    {label}
                  </button>
                ))}
            ...
            {showAction === "admin" ? (
              ...
              >
                ...
                {adminSaving
                  ? "Attribution..."
                  : "Attribuer l'offre (sans paiement)"}
              </button>
            ) : (
              <button
                type="button"
                onClick={() => {
                  if (showAction === "changer" && !nouvelleOffreId) {
                    setActionError("Veuillez sélectionner une offre.");
                    return;
                  }
                  setActionError("");
