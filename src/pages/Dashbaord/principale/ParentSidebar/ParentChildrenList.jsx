  ...
  const handleRemoveChild = async () => {
    ...
    try {
      ...
      const token =
        ...
        localStorage.getItem("authToken");
      await fetch(
        `${process.env.REACT_APP_API_BASE_URL}/parents/${parentId}/enfants/${confirmDeleteId}`,
        {
          method: "DELETE",
          headers: {
            Authorization: `Bearer ${token}`,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              {children.length} enfant(s) associe(s)
            </p>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-16 bg-white rounded-2xl border border-gray-200">
          ...
          <p className="text-gray-500 mb-6 text-sm">
            Ajoutez vos enfants pour pouvoir les inscrire dans des classes.
          </p>
          <button
            onClick={() => setShowAddModal(true)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {children.map((child) => (
            ...
            >
              ...
              {child.telephone && (
                <p className="text-xs text-gray-500">{child.telephone}</p>
              )}
              <div className="mt-3 pt-3 border-t border-gray-100">
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${child.etat === "ACTIVE" ? "bg-green-50 text-green-700" : "bg-amber-50 text-amber-700"}`}
                >
                  {child.etat === "ACTIVE"
                    ? "Actif"
                    : child.etat || "En attente"}
                </span>
              </div>
            </div>
          ))}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[9999] p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm p-6">
            ...
            <p className="text-sm text-gray-500 mb-6">
              Cet enfant sera retiré de votre compte. Cette action ne supprime
              pas son profil.
            </p>
