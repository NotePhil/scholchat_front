...
const StatusBadge = ({ status }) => {
  const cfg = {
    ACTIVE: { cls: "bg-emerald-50 text-emerald-700 border-emerald-200", dot: "bg-emerald-500", label: "Actif" },
    INACTIVE: { cls: "bg-red-50 text-red-700 border-red-200", dot: "bg-red-500", label: "Inactif" },
    PENDING: { cls: "bg-amber-50 text-amber-700 border-amber-200", dot: "bg-amber-500 animate-pulse", label: "En attente" },
    AWAITING_VALIDATION: { cls: "bg-amber-50 text-amber-700 border-amber-200", dot: "bg-amber-500 animate-pulse", label: "En attente" },
  };
  const c = cfg[status] || { cls: "bg-slate-50 text-slate-700 border-slate-200", dot: "bg-slate-400", label: status || "—" };
  return (
...
const UserViewModalParentStudent = ({ user, onClose }) => {
  ...
  const tabs = [
    ...
    { id: "children", label: `Enfants (${enfants.length})` },
    { id: "classes", label: `Classes (${parentClasses.length})` },
  ];
          ...
          {loading ? (
            <div className="flex justify-center items-center py-16">
              <div className="relative w-10 h-10">
                ...
                <div className="w-10 h-10 border-4 border-blue-600 rounded-full animate-spin absolute top-0 left-0"
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-slate-50 rounded-xl p-4 sm:p-5">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <InfoRow icon={Mail} iconBg="bg-blue-50" iconColor="text-blue-600" label="Email" value={user?.email} />
                      <InfoRow icon={Phone} iconBg="bg-green-50" iconColor="text-green-600" label="Téléphone" value={user?.telephone} />
                      <InfoRow icon={MapPin} iconBg="bg-purple-50" iconColor="text-purple-600" label="Adresse" value={user?.adresse} />
                      <InfoRow
                        icon={Calendar}
                        iconBg="bg-amber-50"
                        ...
                        value={user?.creationDate
                          ? new Date(user.creationDate).toLocaleDateString("fr-FR", { year: "numeric", month: "long", day: "numeric" })
                          : null}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 text-center">
                      ...
                      <p className="text-lg font-bold text-blue-700">{enfants.length}</p>
                      <p className="text-xs text-blue-600">Enfants</p>
                    </div>
                    <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-3 text-center">
                      <School className="w-5 h-5 text-indigo-600 mx-auto mb-1" />
                      <p className="text-lg font-bold text-indigo-700">{parentClasses.length}</p>
                    ...
                    <div className={`rounded-xl p-3 text-center border ${
                      ...
                      <GraduationCap className={`w-5 h-5 mx-auto mb-1 ${
                        user?.etat === "ACTIVE" ? "text-emerald-600" : "text-red-600"
                      }`} />
                      <p className={`text-lg font-bold ${
                        user?.etat === "ACTIVE" ? "text-emerald-700" : "text-red-700"
                      }`}>
                ...
                <div className="space-y-3">
                  {enfants.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-600 font-medium">Aucun enfant enregistré</p>
                      <p className="text-slate-400 text-sm mt-1">Ce parent n'a pas encore d'enfants associés.</p>
                    </div>
                  ) : (
                    enfants.map((child) => {
                      const classes = childClasses[child.id] || [];
                      return (
                        <div key={child.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                          <div className="flex items-start gap-3">
                ...
                <div className="space-y-3">
                  {parentClasses.length === 0 ? (
                    <div className="text-center py-12">
                      ...
                      <p className="text-slate-600 font-medium">Aucune classe associée</p>
                      <p className="text-slate-400 text-sm mt-1">Ce parent n'a accès à aucune classe pour l'instant.</p>
                    </div>
                  ) : (
                    parentClasses.map((cls) => (
                      <div key={cls.id} className="bg-white border border-slate-200 rounded-xl p-4 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between gap-3">
                          <div className="flex items-start gap-3">
                            ...
                            <div>
                              <h4 className="font-semibold text-slate-900 text-sm">{cls.nom}</h4>
                              {cls.niveau && (
                                <p className="text-xs text-slate-500 mt-0.5 flex items-center gap-1">
                                  <GraduationCap className="w-3 h-3" /> {cls.niveau}
                                </p>
                              )}
                              {cls.codeActivation && (
                                <p className="text-xs text-slate-400 mt-0.5 flex items-center gap-1">
                                  <Hash className="w-3 h-3" /> {cls.codeActivation}
                                </p>
                              )}
                              {cls.etablissement && (
                          ...
                          {cls.etat && (
                            <span className={`text-xs px-2 py-0.5 rounded-full flex-shrink-0 ${
                              cls.etat === "ACTIF" ? "bg-emerald-100 text-emerald-700" : "bg-amber-100 text-amber-700"
                            }`}>
                              {cls.etat}
                            </span>
                          )}
