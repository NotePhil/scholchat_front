...
const UserViewModalParentStudent = ({ user, onClose, userType = "parent" }) => {
  ...
  const fetchUserData = async () => {
    try {
      ...
      // Fetch students if parent has classes
      if (userType === "parent" && user.classes && user.classes.length > 0) {
        const allStudents = [];
  ...
  return (
    <>
      {/* Modal structure exactly matching CreateCourseFormModal */}
          ...
          <div className="p-4 sm:p-6 border-b border-slate-200 flex-shrink-0 sticky top-0 bg-white rounded-t-2xl z-10">
            <div className="flex items-center justify-between">
              <h2 className="text-xl sm:text-2xl font-bold text-slate-900 flex items-center">
                ...
                <span className="sm:hidden">Profil Parent</span>
              </h2>
