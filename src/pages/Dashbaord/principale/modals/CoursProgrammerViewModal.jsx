...
const CoursProgrammerViewModal = ({
  ...
  useEffect(() => {
    ...
    userService
      .getUserById(scheduledCourse.professeurId)
      .then((user) => {
        if (user) {
          setProfessorName(
  ...
  useEffect(() => {
    const fetchParticipants = async () => {
      ...
      console.log(
        "[CoursProgrammerViewModal] Fetching participants for course:",
        {
        ...
        if (
          ...
          try {
            ...
            if (classUsersResponse.ok) {
              ...
              console.log(
                "[CoursProgrammerViewModal] Raw class users response:",
                classUsers,
              );
              ...
              expectedParticipants = classUsers
                .filter((user) => {
                  ...
                  console.log(
                    `[CoursProgrammerViewModal] User ${user.id} (${user.prenom} ${user.nom}) type: ${userType}, included: ${isNotProfessor}`,
                  );
              ...
              console.log(
                "[CoursProgrammerViewModal] Filtered expected participants:",
                expectedParticipants,
              );
            } else {
              console.error(
                "[CoursProgrammerViewModal] Failed to fetch class users:",
                classUsersResponse.status,
                classUsersResponse.statusText,
              );
            }
          } catch (error) {
            console.error(
              "[CoursProgrammerViewModal] Error fetching class students:",
              error,
            );
          }
        }
        const validParticipants = expectedParticipants.sort((a, b) =>
    ...
    const fetchSessionHistory = async () => {
      ...
      try {
        ...
        console.log(
          "[AttendanceModal] Fetching session history for course:",
          scheduledCourse.cours.id,
        );
        const sessions = await liveSessionService.getCourseSessionHistory(
          scheduledCourse.cours.id,
        );
        ...
        console.error(
          "[AttendanceModal] Error fetching session history:",
          error,
        );
        setSessionHistory([]);
      } finally {
        ...
        <div className="bg-gradient-to-r from-slate-50 to-blue-50 px-4 sm:px-8 py-4 sm:py-6 border-b border-slate-200">
          <div className="flex justify-between items-start gap-3">
            ...
            <div className="flex items-center space-x-2 sm:space-x-4 flex-shrink-0">
              <span
                className={`inline-flex items-center px-2 sm:px-3 py-1 rounded-full text-xs sm:text-sm font-medium border ${getStatusBadge(scheduledCourse.etatCoursProgramme)}`}
              >
                    ...
                    <div className="flex items-center space-x-3 sm:space-x-6">
                      <div className="relative">
                        ...
                        <div className="absolute -bottom-2 -right-2">
                          <div
                            className={`w-6 h-6 rounded-full border-4 border-white ${scheduledCourse.etatCoursProgramme === "PLANIFIE" ? "bg-blue-500" : scheduledCourse.etatCoursProgramme === "EN_COURS" ? "bg-green-500" : scheduledCourse.etatCoursProgramme === "TERMINE" ? "bg-gray-500" : "bg-red-500"}`}
                          ></div>
                        </div>
                      </div>
                      <div>
                    ...
                    <div className="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                      {classDetails ? (
                        <div className="flex items-start justify-between">
                          ...
                          <span
                            className={`text-xs px-2 py-1 rounded-full ${classDetails.etat === "ACTIF" ? "bg-emerald-100 text-emerald-800" : "bg-amber-100 text-amber-800"}`}
                          >
                            {classDetails.etat}
                          </span>
                        </div>
                      ...
                      <div className="flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-xl p-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-amber-600 mt-0.5">
                            Ce cours a été programmé par un autre professeur de
                            la classe
                          </p>
                        </div>
                      </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      ...
                      <div className="bg-white rounded-lg p-4 shadow-sm">
                        <div className="text-2xl font-bold text-green-600">
                          {
                            sessionHistory.filter((s) => s.status === "ENDED")
                              .length
                          }
                        </div>
                        <div className="text-sm text-slate-600">
                    ...
                    <div className="bg-white rounded-xl border border-slate-200 shadow-sm">
                      <div className="p-6 border-b border-slate-200">
                        <h4 className="text-lg font-semibold text-slate-900 flex items-center">
                          ...
                          scheduledCourse.participantsIds.length > 0 ? (
                            <span>
                              Participants Sélectionnés ({participants.length})
                            </span>
                          ) : (
                            <span>
                      ...
                      {participants.length === 0 ? (
                        ...
                        <div className="divide-y divide-slate-200">
                          {participants.map((participant) => {
                            ...
                            const attendedSessions = sessionHistory.filter(
                              (session) =>
                            ...
                            const attendanceRate =
                              totalSessions > 0
                                ? Math.round(
                                    (attendedSessions.length / totalSessions) *
                                      100,
                                  )
                                : 0;
                            console.log(
                              `[AttendanceModal] Participant ${participant.name}:`,
                              {
                                id: participant.id,
                                attendedSessions: attendedSessions.length,
                                totalSessions: totalSessions,
                                attendanceRate: attendanceRate + "%",
                                sessionsAttended: attendedSessions.map(
                                  (s) => s.sessionId,
                                ),
                              },
                            );
                                ...
                                <div className="flex items-start justify-between">
                                  ...
                                  <div className="flex-shrink-0 text-right">
                                    <div className="flex items-center space-x-2 mb-2">
                                      ...
                                      <span
                                        className={`text-sm font-medium ${attendanceRate >= 80 ? "text-green-700" : attendanceRate >= 50 ? "text-orange-700" : "text-red-700"}`}
                                      >
                                    ...
                                    {totalSessions > 0 && (
                                      <div className="mt-2">
                                        <div className="w-20 bg-slate-200 rounded-full h-2">
                                          <div
                                            className={`h-2 rounded-full transition-all duration-300 ${attendanceRate >= 80 ? "bg-green-500" : attendanceRate >= 50 ? "bg-orange-500" : "bg-red-500"}`}
                                            style={{
                          ...
                          {sessionHistory.map((session, index) => (
                            <div key={session.sessionId} className="p-4">
                              <div className="flex items-center justify-between">
                                <div className="flex items-center space-x-3">
                                  <div
                                    className={`w-3 h-3 rounded-full ${session.status === "ENDED" ? "bg-green-500" : session.status === "ACTIVE" ? "bg-blue-500" : "bg-gray-500"}`}
                                  ></div>
                                  <div>
                                ...
                                <div className="text-right">
                                  ...
                                  <div
                                    className={`text-xs px-2 py-1 rounded-full ${session.status === "ENDED" ? "bg-green-100 text-green-700" : session.status === "ACTIVE" ? "bg-blue-100 text-blue-700" : "bg-gray-100 text-gray-700"}`}
                                  >
                ...
                onClick={() => {
                  ...
                  if (
                    ...
                    const reprogramData = {
                      ...
                      description: scheduledCourse.description?.includes(
                        "Annulé:",
                      )
                        ? null
                        : scheduledCourse.description,
