...
const LayoutDashboard = asIconComponent(faTableColumns);
const MessageSquare = asIconComponent(faMessage);
const Plus = asIconComponent(faPlus);
const PlusCircle = asIconComponent(faCirclePlus);
...
const MobileBottomNav = ({
  ...
  return (
    <>
            ...
            >
              <div className="flex justify-between items-center mb-8">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    Raccourcis vers vos tâches quotidiennes
                  </p>
                </div>
                <button
                  onClick={() => setShowQuickActions(false)}
              ...
              <div className="grid grid-cols-3 gap-3">
                {quickActions.map((action, index) => (
                  ...
                  >
                    <div
                      className={`p-3 rounded-xl ${action.color} text-white shadow-lg mb-2 group-hover:scale-110 transition-transform`}
                    >
                      <action.icon size={18} />
                    </div>
                    <span className="font-bold text-[10px] dark:text-white text-center line-clamp-1">
                      {action.label}
                    </span>
                  </motion.button>
                ))}
      ...
      {/* Main Bottom Nav Bar */}
      <nav
        className="fixed bottom-0 left-0 right-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-2xl border-t border-gray-100 dark:border-white/5 z-[999] flex items-center justify-around px-4"
        style={{
          height: "calc(80px + env(safe-area-inset-bottom, 16px))",
      ...
      >
        {navItems.map((item, index) => {
          ...
          return (
            ...
            >
              <div
                className={`relative p-2 rounded-xl transition-all duration-300 ${active ? "scale-110" : "text-gray-400 opacity-60"}`}
              >
                <item.icon
                  size={24}
              ...
              <span
                className={`text-[10px] font-black uppercase tracking-tighter ${active ? "opacity-100 transform -translate-y-1" : "opacity-0"} transition-all duration-300`}
                style={{
                  color: primaryColor,
                }}
              >
