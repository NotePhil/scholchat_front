...
const RenewalPage = ({ theme }) => {
  ...
  return (
    <div
      className={`min-h-screen flex items-center justify-center py-8 sm:py-12 px-4 ${theme === "dark" ? "bg-gray-900" : "bg-gray-50"}`}
    >
      <div className="max-w-md w-full">
        ...
        {token ? (
          <RenewalWithToken token={token} theme={theme} />
        ) : (
          <RenewalRequestForm />
        )}
      </div>
...
const RenewalRequestForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (
      !form.email.trim() ||
      (!form.classeId.trim() && !form.etablissementId.trim())
    ) {
      setError(
        "Merci de renseigner votre email et l'identifiant de votre classe ou établissement.",
      );
      return;
    }
    setLoading(true);
    ...
    return (
      ...
      >
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          Si les informations correspondent à un compte existant, un email
          contenant un lien de renouvellement vient de vous être envoyé.
        </p>
      </motion.div>
    );
  }
  return (
    ...
    >
      ...
      <p className="text-sm text-gray-500 dark:text-gray-400 mb-6">
        Renseignez votre email et l'identifiant de votre classe ou
        établissement. Vous recevrez un lien sécurisé.
      </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          <div className="flex gap-2">
            {[
              ["CLASSE", "ID de classe"],
              ["ETABLISSEMENT", "ID d'établissement"],
            ].map(([val, label]) => (
              <button
                key={val}
                type="button"
                onClick={() => setEntityType(val)}
                className={`flex-1 px-3 py-2.5 rounded-xl border text-sm font-medium transition-colors ${entityType === val ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
              >
                {label}
              </button>
            ))}
        ...
        <div>
          ...
          <input
            type="text"
            value={
              entityType === "CLASSE" ? form.classeId : form.etablissementId
            }
            onChange={(e) =>
              setForm((p) =>
                entityType === "CLASSE"
...
const RenewalWithToken = ({ token, theme }) => {
  ...
  useEffect(() => {
    const charger = async () => {
      ...
      try {
        ...
        setStatut(data);
        if (data?.contratCourant?.periodicite)
          setPeriodicite(data.contratCourant.periodicite);
      } catch {
        setError(
          "Ce lien de renouvellement est invalide ou a expiré. Merci de refaire une demande.",
        );
      } finally {
        setLoading(false);
      }
  ...
  const offreCible =
    (statut?.offresDisponibles || []).find((o) => o.id === nouvelleOffreId) ||
    null;
  const montant =
    action === "changer" && offreCible
      ...
      : statut?.contratCourant
        ? Number(
            periodicite === PeriodiciteContrat.ANNUEL
              ? statut.contratCourant.prixAnnuel
              : statut.contratCourant.prixMensuel,
          ) || 0
        : 0;
  const handlePaymentSuccess = async (paymentInfo) => {
    ...
    try {
      ...
      if (action === "changer")
        await contratService.changerOffreParToken(token, payload);
      ...
      setPayError(
        err.response?.data?.message ||
          "Une erreur est survenue lors du renouvellement.",
      );
    }
  };
  if (loading)
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8 text-center text-gray-500 animate-pulse">
        Chargement...
      </div>
    );
  if (error && !statut)
  ...
  if (success)
    return (
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 text-center">
        ...
        <p className="text-gray-600 dark:text-gray-400 text-sm">
          Votre offre a été renouvelée avec succès. Vous pouvez maintenant vous
          connecter.
        </p>
      </div>
    );
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-6 sm:p-8 space-y-5">
      ...
      {contrat && (
        <div className="bg-gray-50 dark:bg-gray-700 rounded-xl p-4 space-y-2 text-sm">
          ...
          <div className="flex justify-between text-gray-600 dark:text-gray-300">
            <span>Statut</span>
            <span
              className={`font-semibold ${contrat.statut === "ACTIF" ? "text-green-600" : "text-red-600"}`}
            >
              {contrat.statut}
            </span>
          </div>
      ...
      <div className="flex gap-2">
        {[
          ["prolonger", "Prolonger la même offre"],
          ...
          <button
            ...
            className={`flex-1 px-3 py-2.5 rounded-xl border text-sm font-medium transition-colors ${action === val ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
          >
            {label}
          </button>
        ))}
      ...
      <div className="flex gap-2">
        {[
          ...
          <button
            ...
            className={`flex-1 px-3 py-2.5 rounded-xl border text-sm font-medium transition-colors ${periodicite === val ? "bg-blue-600 border-blue-600 text-white" : "border-gray-300 text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700"}`}
          >
            {label}
          </button>
        ))}
      ...
      <PaymentModal
        ...
        label={
          action === "changer"
            ...
            : contrat?.offreNom || "Renouvellement"
        }
        subLabel={
          periodicite === "ANNUEL"
