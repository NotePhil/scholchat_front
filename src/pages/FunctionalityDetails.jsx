...
const FunctionalityDetails = ({ theme }) => {
  ...
  const iconMap = {
    schoolAdmin: { icon: Building2, color: "text-blue-500", bg: "bg-blue-500/10" },
    studentSuccess: { icon: GraduationCap, color: "text-green-500", bg: "bg-green-500/10" },
    parentEngagement: { icon: Users, color: "text-purple-500", bg: "bg-purple-500/10" },
    securePlatform: { icon: ShieldCheck, color: "text-red-500", bg: "bg-red-500/10" },
    analytics: { icon: TrendingUp, color: "text-amber-500", bg: "bg-amber-500/10" },
    curriculum: { icon: BookOpen, color: "text-teal-500", bg: "bg-teal-500/10" },
  };
    ...
    return (
      <div className={`min-h-screen flex items-center justify-center ${themeClasses.bg} ${themeClasses.text}`}>
        <div className="text-center">
          <h2 className="text-2xl font-bold mb-4">{t("pages.functionalities.details.notFound.title")}</h2>
          <button 
          ...
          >
            <ArrowLeft size={20} /> {t("pages.functionalities.details.notFound.goBack")}
          </button>
        </div>
      </div>
    );
  ...
  const benefits = t(`${translationBase}.benefits`, { returnObjects: true });
    ...
    >
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <span className="font-medium">{t("pages.functionalities.details.backButton")}</span>
        </motion.button>
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-start">
          ...
          >
            <div className={`inline-flex p-6 rounded-3xl ${featureConfig.bg} mb-8`}>
            ...
            <div className={`text-xl leading-relaxed ${themeClasses.subtext} mb-8`}>
              {t(`pages.functionalities.features.${id}.desc`)} 
            </div>

             <div className="hidden lg:block">
                 <div className={`p-8 rounded-3xl ${themeClasses.cardBg} border ${themeClasses.border} shadow-lg`}>
                    <h3 className="text-xl font-bold mb-4">{t("pages.functionalities.details.whyChoose.title")}</h3>
            ...
            <div className={`p-8 rounded-3xl ${themeClasses.cardBg} border ${themeClasses.border} shadow-lg`}>
              ...
              <ul className="space-y-4">
                {benefitsList.map((benefit, index) => (
                  ...
                  >
                    <div className={`mt-1 p-1 rounded-full ${featureConfig.bg}`}>
                      <CheckCircle2 className={`w-5 h-5 ${featureConfig.color}`} />
                    </div>
                    <span className="text-base sm:text-lg opacity-90 break-words flex-1">{benefit}</span>
                  </motion.li>
                ))}
              </ul>
            </div>
