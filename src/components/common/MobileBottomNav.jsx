...
const MobileBottomNav = () => {
  ...
  const navItems = [
    { icon: Home, label: "Home", path: "/schoolchat/principal/etablissement" },
    { icon: MessageSquare, label: "Messages", path: "/schoolchat/messages", hasSubMenu: true },
    { icon: Plus, label: "Action", isCenter: true },
    { icon: Bell, label: "Alerts", path: "/schoolchat/notifications" },
    { icon: User, label: "Profile", path: "/schoolchat/profile" },
  ];

  const quickActions = [
    { icon: BookOpen, label: "New Class", color: "bg-blue-500" },
            ...
            >
              ...
              <div className="grid grid-cols-2 gap-4">
                {quickActions.map((action, index) => (
                  ...
                  >
                    <div className={`p-3 rounded-xl ${action.color} text-white shadow-lg shadow-blue-500/20`}>
                      <action.icon size={20} />
                    </div>
                    <span className="font-semibold dark:text-white">{action.label}</span>
                  </motion.button>
                ))}
              </div>
            </motion.div>
      ...
      <nav className="fixed bottom-6 left-6 right-6 h-20 bg-white/90 dark:bg-gray-900/90 backdrop-blur-xl border border-white/20 dark:border-gray-800 rounded-[32px] shadow-2xl z-[90] flex items-center justify-around px-2">
        {navItems.map((item, index) => {
          ...
          return (
            ...
            >
              <div className={`p-2 rounded-2xl transition-all duration-300 ${active ? 'bg-blue-50 dark:bg-blue-900/20 text-blue-600' : 'text-gray-400 group-hover:text-gray-600'}`}>
              ...
              <span className={`text-[10px] font-bold uppercase tracking-widest ${active ? 'text-blue-600' : 'text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity'}`}>
                {item.label}
              </span>
              {active && (
                <motion.div
